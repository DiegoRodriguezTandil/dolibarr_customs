...
/**
 *  \file       htdocs/salesorder/class/salesorder.class.php
 *  \ingroup    salesorder
 *  \brief      Fichier des classes de salesorders
 */
 ...
 */
class salesorder extends CommonOrder
{
    public $element='salesorder';
    public $table_element='salesorder';
    public $table_element_line = 'salesorderdet';
    public $class_element_line = 'SalesOrderLine';
    public $fk_element = 'fk_salesorder';
    ...
    var $adresse;
    var $date;				// Date salesorder
    var $date_salesorder;		// Date salesorder (deprecated)
    ...
    function getNextNumRef($soc)
    {
        ...
        $dir = DOL_DOCUMENT_ROOT . "/core/modules/salesorder";

        if (! empty($conf->global->SALESORDER_ADDON))
        {
            $file = $conf->global->SALESORDER_ADDON.".php";

            // Chargement de la classe de numerotation
            $classname = $conf->global->SALESORDER_ADDON;
            ...
            if ($result)
            {
                ...
                {
                    dol_print_error($db,"salesorder::getNextNumRef ".$obj->error);
                    return "";
                }
            }
            else
            {
                print $langs->trans("Error")." ".$langs->trans("Error_SALESORDER_ADDON_NotDefined");
                return "";
            }
        }
        else
    ...
    function valid($user, $idwarehouse=0)
    {
        ...
        if (! $user->rights->salesorder->valider)
        {
        ...
        // Definition du nom de module de numerotation de salesorder
        ...
        // Validate
        $sql = "UPDATE ".MAIN_DB_PREFIX."salesorder";
        $sql.= " SET ref = '".$num."',";
        $sql.= " fk_statut = 1,";
        ...
        if (! $error)
        {
            ...
            if (preg_match('/^[\(]?PROV/i', $this->ref))
            {
                ...
                $dirsource = $conf->salesorder->dir_output.'/'.$comref;
                $dirdest = $conf->salesorder->dir_output.'/'.$snum;
                if (file_exists($dirsource))
                {
                    ...
                    if (@rename($dirsource, $dirdest))
                    {
                        ...
                        dol_delete_file($conf->salesorder->dir_output.'/'.$snum.'/'.$comref.'*.*');
                    }
                }
            }
        }
        ...
        if (! $error)
        {
            ...
            $result=$interface->run_triggers('SALESORDER_VALIDATE',$this,$user,$langs,$conf);
    ...
    function set_reopen($user)
    {
        ...
        dol_syslog("salesorder::set_reopen sql=".$sql);
        $resql = $this->db->query($sql);
        if ($resql)
        {
            // Appel des triggers
	...
	function cancel($idwarehouse=-1)
	{
		...
		dol_syslog("salesorder::cancel sql=".$sql, LOG_DEBUG);
		if ($this->db->query($sql))
		{
			// If stock is decremented on validate order, we must reincrement it
			if (! empty($conf->stock->enabled) && $conf->global->STOCK_CALCULATE_ON_VALIDATE_ORDER == 1)
    ...
    function create($user, $notrigger=0)
    {
        ...
        // Clean parameters
        $this->brouillon = 1;		// On positionne en mode brouillon la salesorder

        dol_syslog("salesorder::create user=".$user->id);
        ...
        if ($result < 0)
        {
            ...
            dol_syslog("salesorder::create ".$this->error, LOG_ERR);
            return -2;
        }
        if (! empty($conf->global->SALESORDER_REQUIRE_SOURCE) && $this->source < 0)
        {
        ...
        // $date_salesorder is deprecated
        $date = ($this->date_salesorder ? $this->date_salesorder : $this->date);

        $now=dol_now();

        $this->db->begin();

        $sql = "INSERT INTO ".MAIN_DB_PREFIX."salesorder (";
        $sql.= " ref, office,fk_soc, date_creation, fk_user_author, fk_projet, fk_currency, date_salesorder, source, note, note_public, ref_client, ref_int";
        ...
        if ($resql)
        {
            $this->id = $this->db->last_insert_id(MAIN_DB_PREFIX.'salesorder');
            ...
            if ($this->id)
            {
                ...
                // Mise a jour ref
                $sql = 'UPDATE '.MAIN_DB_PREFIX."salesorder SET ref='(PROV".$this->id.")' WHERE rowid=".$this->id;
                if ($this->db->query($sql))
                {
                    if ($this->id)
                    {
                        ...
                        if (is_array($this->linked_objects) && ! empty($this->linked_objects))
                        {
                        	foreach($this->linked_objects as $origin => $origin_id)
                        	{
                        		...
                        		if ($origin == 'propal' && $origin_id)
                        		{
                        			...
                        			if ($this->userid)
                        			{
                        				//On passe le commercial suivi propale en commercial suivi salesorder
                        			...
                        			if ($this->contactid)
                        			{
                        				//On passe le contact client suivi propale en contact client suivi salesorder
    ...
    function createFromProposal($object)
    {
        ...
        if ($object->statut == 2)
        {
            $this->date_salesorder = dol_now();
            $this->source = 0;

            $num=count($object->lines);
            for ($i = 0; $i < $num; $i++)
            {
                $line = new SalesOrderLine($this->db);
            ...
            if ($ret > 0)
            {
                ...
                if (! $error)
                {
                    // Ne pas passer par la salesorder provisoire
                    if ($conf->global->SALESORDER_VALID_AFTER_CLOSE_PROPAL == 1)
                    {
                        $this->fetch($ret);
                        $this->valid($user);
                    }
     ...
     *
     *	@param      int				$salesorderid      	Id of line
	...
	function addline($salesorderid, $desc, $pu_ht, $qty, $txtva, $txlocaltax1=0, $txlocaltax2=0, $fk_product=0, $remise_percent=0, $info_bits=0, $fk_remise_except=0, $price_base_type='HT', $pu_ttc=0, $date_start='', $date_end='', $type=0, $rang=-1, $special_code=0, $fk_parent_line=0, $fk_fournprice=null, $pa_ht=0, $label='',$line_ref=null )
    {
        ...
        if ($this->statut == 0)
        {
            ...
            // Insert line
            $this->line=new SalesOrderLine($this->db);

            $this->line->fk_salesorder=$salesorderid;
            ...
            if ($result > 0)
            {
                ...
                // Mise a jour informations denormalisees au niveau de la salesorder meme
                $this->id=$salesorderid;	// TODO A virer
    ...
    function add_product($idproduct, $qty, $remise_percent=0, $date_start='', $date_end='')
    {
        ...
        if ($idproduct > 0)
        {
            ...
            /** POUR AJOUTER AUTOMATIQUEMENT LES SOUSPRODUITS a LA salesorder
    ...
    function fetch($id, $ref='', $ref_ext='', $ref_int='')
    {
        ...
        $sql.= ', c.date_salesorder';
        $sql.= ', c.date_livraison';
		//FEDE 16-10-2012
		$sql.= ', c.fk_currency';
		$sql.= ', c.office';
		$sql.= ', c.shipment_location';
		//FIN FEDE
        ...
        if ($result)
        {
            ...
            if ($obj)
            {
                ...
                $this->date					= $this->db->jdate($obj->date_salesorder);
                $this->date_salesorder		= $this->db->jdate($obj->date_salesorder);
				...
				$this->shipment_location	= $obj->shipment_location;
				//FIN FEDE
	...
	{
		$sql = "SELECT rate FROM llx_currency_conversion a join llx_salesorder b on a.source=b.fk_currency and a.target='".$currency."' and a.date<=b.date_valid where rowid=".$id." order by date desc";
    ...
    function fetch_lines($only_product=0)
    {
        ...
        $sql = 'SELECT l.rowid, l.fk_product, l.fk_parent_line, l.product_type, l.fk_salesorder, l.label as custom_label, l.description,l.fk_currency, l.origin_price, l.price, l.qty, l.tva_tx,';
        ...
        if ($result)
        {
            ...
            while ($i < $num)
            {
                ...
                $line->id               = $objp->rowid;
                $line->fk_salesorder      = $objp->fk_salesorder;
                $line->salesorder_id      = $objp->fk_salesorder;			// \deprecated
				...
				//FEDE
				$line->fk_currency     	= $objp->fk_currency;
				$line->origin_price     = $objp->origin_price;
				$line->cost     		= $objp->cost;
				$this->cost				= $this->cost+$objp->cost;
				//FIN FEDE				
        ...
        {
            $this->error=$this->db->error();
            dol_syslog('salesorder::fetch_lines: Error '.$this->error, LOG_ERR);
            return -3;
        }
    }
    ...
    function loadExpeditions($filtre_statut=-1)
    {
        ...
        $sql.= ' '.MAIN_DB_PREFIX.'salesorderdet as cd';
        $sql.= ' WHERE';
        if ($filtre_statut >= 0) $sql.= ' ed.fk_expedition = e.rowid AND';
        $sql.= ' ed.fk_origin_line = cd.rowid';
        $sql.= ' AND cd.fk_salesorder =' .$this->id;
        if ($filtre_statut >= 0) $sql.=' AND e.fk_statut = '.$filtre_statut;
        $sql.= ' GROUP BY cd.rowid, cd.fk_product';
        //print $sql;

        dol_syslog("salesorder::loadExpeditions sql=".$sql,LOG_DEBUG);
        $result = $this->db->query($sql);
        if ($result)
        {
            $num = $this->db->num_rows($result);
        ...
        {
            $this->error=$this->db->lasterror();
            dol_syslog("salesorder::loadExpeditions ".$this->error,LOG_ERR);
            return -1;
        }
    ...
    function stock_array($filtre_statut=-1)
    {
        ...
        // Tableau des id de produit de la salesorder
    ...
    function deleteline($lineid)
    {
        ...
        if ($this->statut == 0)
        {
            ...
            $sql.= " FROM ".MAIN_DB_PREFIX."salesorderdet";
    ...
    function set_remise_absolue($user, $remise)
    {
        ...
        if ($user->rights->salesorder->creer)
        {
            ...
            dol_syslog("salesorder::set_remise_absolue sql=$sql");
    ...
    function set_date($user, $date)
    {
        ...
        {
            $sql = "UPDATE ".MAIN_DB_PREFIX."salesorder";
            $sql.= " SET date_salesorder = ".($date ? $this->db->idate($date) : 'null');
            $sql.= " WHERE rowid = ".$this->id." AND fk_statut = 0";

            dol_syslog("salesorder::set_date sql=$sql",LOG_DEBUG);
            $resql=$this->db->query($sql);
            if ($resql)
            {
                $this->date = $date;
            ...
            {
                $this->error=$this->db->error();
                dol_syslog("salesorder::set_date ".$this->error,LOG_ERR);
                return -1;
            }
        }
        else
	...
	//FEDE
    function setShipmentLocation($location)
    {
		$sql = "UPDATE ".MAIN_DB_PREFIX."salesorder";
		$sql.= " SET shipment_location = '".$location."'";
		$sql.= " WHERE rowid = ".$this->id;

		dol_syslog("Salesorder::setShipmentLocation sql=$sql",LOG_DEBUG);
		$resql=$this->db->query($sql);
		if ($resql)
		{
			$this->shipment_location = $location;
			return 1;
		}
		else
		{
			$this->error=$this->db->error();
			dol_syslog("Salesorder::setShipment_location ".$this->error,LOG_ERR);
			return -1;
		}
    }
	//FIN FEDE
    ...
    function set_date_livraison($user, $date_livraison)
    {
        if ($user->rights->salesorder->creer)
        {
            ...
            dol_syslog("salesorder::set_date_livraison sql=".$sql,LOG_DEBUG);
            $resql=$this->db->query($sql);
            if ($resql)
            {
            ...
            {
                $this->error=$this->db->error();
                dol_syslog("salesorder::set_date_livraison ".$this->error,LOG_ERR);
                return -1;
            }
        }
        else
    ...
    function set_availability($user, $id)
    {
        if ($user->rights->salesorder->creer)
        {
            ...
            {
                $this->error=$this->db->error();
                dol_syslog("salesorder::set_availability Erreur SQL");
                return -1;
            }
        }
    }
    ...
    function set_demand_reason($user, $id)
    {
        if ($user->rights->salesorder->creer)
        {
            ...
            {
                $this->error=$this->db->error();
                dol_syslog("salesorder::set_demand_reason Erreur SQL");
                return -1;
            }
        }
    }
    ...
    function liste_array($brouillon=0, $user='')
    {
        ...
        $sql.= " FROM ".MAIN_DB_PREFIX."societe as s, ".MAIN_DB_PREFIX."salesorder as c";
        $sql.= " WHERE c.entity = ".$conf->entity;
        $sql.= " AND c.fk_soc = s.rowid";
        if ($brouillon) $sql.= " AND c.fk_statut = 0";
        if ($user) $sql.= " AND c.fk_user_author <> ".$user->id;
        $sql .= " ORDER BY c.date_salesorder DESC";
    ...
    {
        dol_syslog('salesorder::availability('.$availability_id.')');
        if ($this->statut >= 0)
        {
            $sql = 'UPDATE '.MAIN_DB_PREFIX.'salesorder';
            ...
            {
                dol_syslog('salesorder::availability Erreur '.$sql.' - '.$this->db->error(), LOG_ERR);
                $this->error=$this->db->lasterror();
                return -1;
            }
        }
        ...
        {
            dol_syslog('salesorder::availability, etat facture incompatible', LOG_ERR);
            $this->error='Etat salesorder incompatible '.$this->statut;
            return -2;
        }
    }
    ...
    {
        dol_syslog('salesorder::demand_reason('.$demand_reason_id.')');
        if ($this->statut >= 0)
        {
            $sql = 'UPDATE '.MAIN_DB_PREFIX.'salesorder';
            ...
            {
                dol_syslog('salesorder::demand_reason Erreur '.$sql.' - '.$this->db->error(), LOG_ERR);
                $this->error=$this->db->lasterror();
                return -1;
            }
        }
        ...
        {
            dol_syslog('salesorder::demand_reason, etat facture incompatible', LOG_ERR);
            $this->error='Etat salesorder incompatible '.$this->statut;
            return -2;
        }
    }
    ...
    function set_ref_client($user, $ref_client)
    {
        if ($user->rights->salesorder->creer)
        {
            ...
            $sql = 'UPDATE '.MAIN_DB_PREFIX.'salesorder SET';
	...
	function classifyBilled()
	{
		...
		$sql = 'UPDATE '.MAIN_DB_PREFIX.'salesorder SET facture = 1';
		$sql.= ' WHERE rowid = '.$this->id.' AND fk_statut > 0';
	...
	function updateline($rowid, $desc, $pu, $qty, $remise_percent, $txtva, $txlocaltax1=0,$txlocaltax2=0, $price_base_type='HT', $info_bits=0, $date_start='', $date_end='', $type=0, $fk_parent_line=0, $skip_update_total=0, $fk_fournprice=null, $pa_ht=0, $label='', $special_code=0,$line_ref=null)
    {
        ...
        if (! empty($this->brouillon))
        {
            ...
            $staticline=new SalesOrderLine($this->db);
    ...
    function delete($user, $notrigger=0)
    {
        ...
        if (! $error)
        {
        	...
        	$sql = 'DELETE FROM '.MAIN_DB_PREFIX."salesorderdet WHERE fk_salesorder = ".$this->id;
        	...
        	// Delete order
        	$sql = 'DELETE FROM '.MAIN_DB_PREFIX."salesorder WHERE rowid = ".$this->id;
        	...
        	{
        		$dir = $conf->salesorder->dir_output . "/" . $comref ;
    ...
    function load_board($user)
    {
        ...
        if ($resql)
        {
            ...
            {
                $this->nbtodo++;
                if ($obj->fk_statut != 3 && $this->db->jdate($obj->datec) < ($now - $conf->salesorder->client->warning_delay)) $this->nbtodolate++;
            }
            return 1;
        }
        else
    ...
    function getNomUrl($withpicto=0,$option=0,$max=0,$short=0)
    {
        ...
        if (! empty($conf->expedition->enabled) && ($option == 1 || $option == 2)) $url = DOL_URL_ROOT.'/expedition/shipment2.php?id='.$this->id;
        else $url = DOL_URL_ROOT.'/salesorder/fiche.php?id='.$this->id;
    ...
    /**
     *	Charge les informations d'ordre info dans l'objet salesorder
     *
    ...
    function load_state_board()
    {
        ...
        $sql.= " FROM ".MAIN_DB_PREFIX."salesorder as co";
...
/**
 *  \class      OrderLine
 *  \brief      Classe de gestion des lignes de salesorder
 */
class SalesOrderLine
{
    ...
    {
        $sql = 'SELECT cd.rowid, cd.fk_salesorder, cd.fk_parent_line, cd.fk_product, cd.product_type, cd.label as custom_label, cd.description, cd.price, cd.qty, cd.tva_tx, cd.localtax1_tx, cd.localtax2_tx,';
    ...
    function delete()
    {
        ...
        $sql = 'DELETE FROM '.MAIN_DB_PREFIX."salesorderdet WHERE rowid='".$this->rowid."';";

        dol_syslog("SalesOrderLine::delete sql=".$sql);
        $resql=$this->db->query($sql);
        if ($resql)
        {
            // Appel des triggers
        ...
        {
            $this->error=$this->db->lasterror();
            dol_syslog("SalesOrderLine::delete ".$this->error, LOG_ERR);
            return -1;
        }
    }
    ...
    function insert($notrigger=0)
    {
        ...
        dol_syslog("SalesOrderLine::insert rang=".$this->rang);
       ...
       // if (empty($this->line_ref)) $this->line_ref=0;
        ...
        $sql = 'INSERT INTO '.MAIN_DB_PREFIX.'salesorderdet';
		...
		//FEDE 16-10-2013
		$sql.= "(SELECT fk_currency FROM llx_product where rowid=".$this->fk_product."),";
		//FIN FEDE
        $sql.= " '".$this->product_type."',";
        $sql.= " '".price2num($this->remise_percent)."',";
		//FEDE 16-10-2013
		$sql.= "(select price from llx_product_price where rowid=(select max(rowid) from llx_product_price where fk_product=".$this->fk_product." and price_level=(SELECT ifnull(a.price_level,1) FROM llx_societe a join llx_salesorder b on a.rowid=b.fk_soc where b.rowid=".$this->fk_salesorder."))),";		
		//FIN FEDE
        ...
        if ($resql)
        {
            $this->rowid=$this->db->last_insert_id(MAIN_DB_PREFIX.'salesorderdet');
	...
	function update($notrigger=0)
	{
		...
		$sql = "UPDATE ".MAIN_DB_PREFIX."salesorderdet SET";
    ...
    function update_total()
    {
        ...
        dol_syslog("SalesOrderLine::update_total sql=$sql");
        ...
        {
            $this->error=$this->db->error();
            dol_syslog("SalesOrderLine::update_total Error ".$this->error, LOG_ERR);
            $this->db->rollback();
            return -2;
        }
    }
