 ...
 */
class pdf_test extends ModelePDFPropales
{
	var $db;
	var $name;
	var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "test";
		$this->description = $langs->trans('DocModelTestDescription');
		...
		if ($conf->propal->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0 ; $i < $nblignes ; $i++)
				{
					...
					//pdf_writelinedesc($pdf,0,$i,$outputlangs,$this->posxtva-$curX+90,3,$curX,$curY,$hideref,$hidedesc,0,$hookmanager,(($i%2)==0));		
					//FEDE
					//$pdf->SetXY($curX, $curY);
					//$pdf->MultiCell(12,3,$i+1,0,'L');
					pdf_writelinedesc($pdf,$object,$i,$outputlangs,$this->posxtva-$curX-8,3,$curX+7,$curY,$hideref,$hidedesc,0,$hookmanager);					
					...
					if ($pageposafter > $pageposbefore)	// There is a pagebreak
					{
						...
						pdf_writelinedesc($pdf,$object,$i,$outputlangs,$this->posxtva-$curX-8,4,$curX+7,$curY,$hideref,$hidedesc,0,$hookmanager);
					...
					if(($i%2)==1)
					{
						$pdf->Rect($curX,$curY-1, 190, $nexY-$curY+2, 'DF', $style4, array(230, 230, 230));
						$pdf->SetXY($curX, $curY);
						$pdf->MultiCell(4,3,$i+1,0,'R',0);
						pdf_writelinedesc($pdf,$object,$i,$outputlangs,$this->posxtva-$curX-8,3,$curX+7,$curY,$hideref,$hidedesc,0,$hookmanager);	
					}*/
					$pdf->SetXY($curX, $curY);
					$pdf->MultiCell(12,3,$i+1,0,'L',0);
					...
					*/
					...
					$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->postotalht, 3, $total_excl_tax, 0, 'R');
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		//cambiar coordenadas de la leyenda total debajo de la tabla, con + 15 se pasa mas a la derecha
		$pdf->SetXY($col1x + 15, $tab2_top + 0);
		//$pdf->MultiCell($col2x-$col1x, $tab2_hl, $outputlangs->transnoentities("TotalHT"), 0, 'L', 1);
		//RENOMBRAR BASE IMPONIBLE (TOTALHT) A TOTAL (DE DEBAJO DE LA TABLA)
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			...
			else
			{
				...
				// Total TTC
				$index++;
				//COMENTADO PARA SACAR IVA DE DEBAJO DE LA TABLA
				/*
				$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
		...
		*/
		...
		//FEDE
		$userdolibarr=$_SESSION['dol_login'];
		$pdf->Image('/var/web/sistema-gestion/documents/ecm/Firmas/'.$userdolibarr.'.jpg' , 170, 250, 30.00);	
		//FIN FEDE
		$index++;
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop))
		{
			...
			$titre = $outputlangs->transnoentities("AmountInCurrency",$outputlangs->transnoentitiesnoconv("Currency".$object->fk_currency));
			//End - gA - 26-07-2013
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{/* SACAR IVA *2 COMENTADO PARA SACAR LEYENDA IVA Y RECUADRO
		...
		*/}
		...
		//$pdf->line(110, $tab_top, 110, $tab_top + $tab_height);
		$pdf->line(110, $tab_top, 110, $tab_top + $tab_height);

		if (empty($hidetop))
		{
			//$pdf->SetXY($this->posxup-1, $tab_top+1);
			//$pdf->SetXY($this->posxup-16, $tab_top+1);
			//era -10
			$pdf->SetXY($this->posxup-8, $tab_top+1);
			//$pdf->MultiCell($this->posxqty-$this->posxup-1,2, $outputlangs->transnoentities("PriceUHT"),'','C');
			//$pdf->MultiCell(10,2, $outputlangs->transnoentities("PriceUHT"),'','C');
			$pdf->MultiCell(45,2, $outputlangs->transnoentities("PriceUHT"),'','C');
		...
		if (empty($hidetop))
		{
			...
			//RENOMBRAR LEYENDA BASE IMPONIBLE (TOTALHT) A TOTAL (DE LA COLUMNA DE LA TABLA AL LADO DE CANT.)
			$pdf->MultiCell(42,2, $outputlangs->transnoentities("Total"),'','C');
		}
	}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager)
	{
		...
		if ($this->emetteur->logo)
		{
			if (is_readable($logo))
			{
				...
				//$pdf->Image('/var/web/sistema-gestion/documents/ecm/Firmas/encabezado01.png' ,$this->marge_gauche, $posy,130);
				//Mostrar el logo correspondiente de acuerdo a la oficina de ventas
				
				$officex = new Societe($this->db);
			    $officex->fetch($object->office);
	 			$comp= $outputlangs->convToOutputCharset($officex->name);
				...
				$pdf->Image('/var/web/sistema-gestion/documents/ecm/Firmas/encabezado01.png' ,$this->marge_gauche, $posy, 120, 18);	// width=0 (auto)
				
				
				}
				if ($comp == 'Redimec USA LLC') {
				
				$pdf->Image('/var/web/sistema-gestion/documents/ecm/Firmas/encabezado02.png' ,$this->marge_gauche, $posy, 130);	// width=0 (auto)
				
				}
				
				if ($comp == 'Redimec LLC') {
				
				$pdf->Image('/var/web/sistema-gestion/documents/ecm/Firmas/encabezado02.png' ,$this->marge_gauche, $posy, 0, $height);	// width=0 (auto)
				
				}
				
				if ($comp == 'Solutions You Can Trust LLC') {
				
				$pdf->Image('/var/web/sistema-gestion/documents/ecm/Firmas/encabezado03.png' ,$this->marge_gauche, $posy, 0, $height);	// width=0 (auto)
				
				}
				
				//fin mostrar logo correspondiente de acuerdo a la oficina de ventas
		...
		if ($showaddress)
		{
			...
			$carac_emetteur .= pdf_build_address($outputlangs,$office);
			...
			//LINEA DE ABAJO COMENTADA PARA SACAR LEYENDA EMISOR
			...
			//$pdf->MultiCell(80, 4, $office->fetch() , 0, 'L');
			...
			$pdf->MultiCell(80, 4, $outputlangs->convToOutputCharset($office->name), 0, 'L');
			...
			$pdf->SetFont('','', $default_font_size  - 1);
			$pdf->MultiCell(80, 4, $office->country, 0, 'L');
			//
			...
			//CAMBIAR LEYENDA DE "ENVIAR A " A "PARA"
			//$pdf->MultiCell($widthrecbox, 5, $outputlangs->transnoentities("BillTo").":", 0, 'L');
			$pdf->MultiCell($widthrecbox, 5, $outputlangs->transnoentities("For").":", 0, 'L');
	...
	function _pagefoot(&$pdf,$object,$outputlangs,$hidefreetext=0)
	{
		        ...
		        $officefooter = new Societe($this->db);
			    $officefooter->fetch($object->office);
	 			$compfooter= $outputlangs->convToOutputCharset($officefooter->name);
				
				if ($compfooter == 'Redimec SRL') {
				
				$pdf->Image('/var/web/sistema-gestion/documents/ecm/Firmas/pie01.png' ,20, 278, 180);
				
				}
				if ($compfooter == 'Redimec USA LLC') {
				
				//$pdf->Image('' ,$this->marge_gauche, 20, 278, 180);
				
				}
				
				if ($compfooter == 'Redimec LLC') {
				
				//$pdf->Image('' ,20, 278, 180);	
