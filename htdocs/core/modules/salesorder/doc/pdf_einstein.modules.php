...
/**
 *	\file       htdocs/core/modules/salesorder/doc/pdf_einstein.modules.php
 *	\ingroup    salesorder
 *	\brief      Fichier de la classe permettant de generer les salesorders au modele Einstein
 */
...
/**
 *	Classe permettant de generer les salesorders au modele Einstein
 */
class pdf_einstein extends ModelePDFSalesorders
{
    var $db;
    var $name;
    var $description;
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0,$hookmanager=false)
	{
		...
		if ($conf->salesorder->dir_output)
		{
			...
			if ($object->specimen)
			{
				$dir = $conf->salesorder->dir_output;
				$file = $dir . "/SPECIMEN.pdf";
			}
			else
			{
				$objectref = dol_sanitizeFileName($object->ref);
				$dir = $conf->salesorder->dir_output . "/" . $objectref;
			...
			if (file_exists($dir))
			{
				...
				$pdf->SetSubject($outputlangs->transnoentities("SalesOrder"));
				$pdf->SetCreator("Dolibarr ".DOL_VERSION);
				$pdf->SetAuthor($outputlangs->convToOutputCharset($user->getFullName($outputlangs)));
				$pdf->SetKeyWords($outputlangs->convToOutputCharset($object->ref)." ".$outputlangs->transnoentities("SalesOrder"));
		...
		{
			$this->error=$langs->trans("ErrorConstantNotDefined","SALESORDER_OUTPUTDIR");
			return 0;
		}
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			...
			//SACAR COLUMNA IVA - SACAR COLUMNA Y LEYENDA IVA
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager)
	{
		...
		if($object->statut==0 && (! empty($conf->global->SALESORDER_DRAFT_WATERMARK)) )
		{
            pdf_watermark($pdf,$outputlangs,$this->page_hauteur,$this->page_largeur,'mm',$conf->global->SALESORDER_DRAFT_WATERMARK);
		}
		...
		if ($this->emetteur->logo)
		{
			if (is_readable($logo))
			{
			    //Código original if is readable...
				/*$height=pdf_getHeightForLogo($logo);
			    $pdf->Image($logo, $this->marge_gauche, $posy, 0, $height);	// width=0 (auto)*/
				//Mostrar logo de acuerdo a la oficina de ventas
				$officelogo = new Societe($this->db);
			    $officelogo->fetch($object->office);
	 			$comp= $outputlangs->convToOutputCharset($officelogo->name);
				...
				$pdf->Image('/var/web/sistema-gestion/documents/ecm/Firmas/encabezado02.png' ,$this->marge_gauche, $posy,130);
			   ...
			   //Fin mostrar logo de acuerdo a la oficina de ventas
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$title=$outputlangs->transnoentities("SalesOrder");
	...
	function _pagefoot(&$pdf,$object,$outputlangs,$hidefreetext=0)
	{
		...
		//Mostrar o esconder footer de acuerdo a la oficina de ventas
	...
	//
		...
		//FEDE acá se agregan las firmas para el pié de página		
		return pdf_pagefoot($pdf,$outputlangs,'SALESORDER_FREE_TEXT',$this->emetteur,$this->marge_basse,$this->marge_gauche,$this->page_hauteur,$object,0,$hidefreetext);
	}
