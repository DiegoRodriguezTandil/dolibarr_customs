...
else if ($action == 'add' && $user->rights->facture->creer)
{
    ...
    if ($_POST['type'] == 1)
    {
        ...
        if (! $error)
        {
			...
			//FEDE 15-10-2013
			$object->fk_currency  		= $_POST['fk_currency'];
			$object->office				= $_POST['office'];
			//FEDE
			...
			//ingresos brutos
			$object->ingresosbrutos     = $_POST['ingresosbrutos'];
			//
...
else if (($action == 'addline' || $action == 'addline_predef') && $user->rights->facture->creer)
{
    ...
    if (! $error && (GETPOST('qty') >= 0) && (! empty($product_desc) || ! empty($idprod)))
    {
        ...
        else
        {
			//FEDE
			$rate=0;
			$resql=$db->query("SELECT rate FROM llx_currency_conversion a join llx_product b on a.source=b.fk_currency join llx_facture c on a.target=c.fk_currency  where  b.rowid=".$idprod." and c.rowid=".$object->id." order by date desc");
...
if ($action == 'create')
{
    ...
    if (! empty($origin) && ! empty($originid) && is_object($objectsrc))
    {
		...
		print '<tr><td>'.$langs->trans('Retenciones Ingresos Brutos').'</td><td colspan="2">';
		print '<input type="text" name="ingresosbrutos"/></td></tr>';
		//
...
else if ($id > 0 || ! empty($ref))
{
    ...
    if ($result > 0)
    {
		...
		print '<tr><td>'.$langs->trans('Retenciones Ingresos Brutos').'</td><td align="right" colspan="2" nowrap>'. price($object->ingresosbrutos).'</td>';
		print '<td>'.$langs->trans('Currency'.$object->fk_currency).'</td>';
		//
