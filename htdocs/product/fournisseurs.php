...
if ($action == 'updateprice' && GETPOST('cancel') <> $langs->trans("Cancel"))
{
	...
	if (! $error)
    {
		...
		if (! $error)
		{
			...
			$ret=$product->update_buyprice($quantity, $_POST["price"],  $_POST["price_list"], $user, $_POST["price_base_type"], $supplier, $_POST["oselDispo"], $ref_fourn, $tva_tx, $_POST["charges"], $remise_percent);
			if ($ret < 0)
			{
				$error++;
				$mesg='<div class="error">'.$product->error.'</div>';
...
if ($id || $ref)
{
	...
	if ($result)
	{
		if ($action <> 'edit' && $action <> 're-edit')
		{
			...
			// Reference
			print '<tr>';
			print '<td width="15%">'.$langs->trans("Part Number").'</td><td colspan="2">';
			print $form->showrefnav($product,'ref','',1,'ref');
			print '</td>';
			print '</tr>';

			// Label
			print '<tr><td>'.$langs->trans("Serial Number").'</td><td colspan="2">'.$product->libelle.'</td></tr>';
			...
			if (($action == 'add_price' || $action == 'updateprice' ) && ($user->rights->produit->creer || $user->rights->service->creer))
			{
				...
				// Precio de Lista FEDE
				print '<tr><td class="fieldrequired">'.$langs->trans("Precio de Lista").'</td>';
				print '<td><input class="flat" name="price_list" size="8" value="'.(GETPOST('price_list')?price(GETPOST('price_list')):(isset($product->fourn_price_list)?price($product->fourn_price_list):'')).'">';
				print '</td></tr>';
