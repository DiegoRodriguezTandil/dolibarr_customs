...
if ($user->societe_id == 0)
{
	...
	DOL_DOCUMENT_ROOT."/salesorder/class/salesorder.class.php",
	...
	DOL_URL_ROOT.'/salesorder/liste.php?mainmenu=commercial',
...
// FEDE Number of policies pending
//if (! empty($conf->commande->enabled) && $user->rights->commande->lire)
{
    include_once DOL_DOCUMENT_ROOT.'/core/class/policy.class.php';
    $board=new Policy($db);
    $board->load_board($user,1);
    $board->warning_delay=$conf->commande->client->warning_delay/60/60/24;
    $board->label=$langs->trans("Pólizas pendientes de cancelación");
    $board->url=DOL_URL_ROOT.'/core/liste.php?viewstatut=-3';
    $board->img=img_object($langs->trans("Orders"),"order");
    $rowspan++;
    $dashboardlines[]=$board;
}

// Number of customer sales orders a deal
if (! empty($conf->salesorder->enabled) && $user->rights->salesorder->lire)
{
    ...
    $board=new Salesorder($db);
    $board->load_board($user);
    $board->warning_delay=$conf->salesorder->client->warning_delay/60/60/24;
    $board->label=$langs->trans("SalesOrdersToProcess");
    $board->url=DOL_URL_ROOT.'/salesorder/liste.php?viewstatut=-3';
    $board->img=img_object($langs->trans("SalesOrders"),"salesorder");
    $rowspan++;
    $dashboardlines[]=$board;
}
...
//  $board->label=$langs->trans("PropalsToClose");
	$board->label=$langs->trans("OpenPropals");
...
if (! empty($conf->propal->enabled) && $user->rights->propale->lire)
{
	...
	$board->label=$langs->trans("ApprovedPropals");
