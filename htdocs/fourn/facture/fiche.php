...
elseif ($action == 'addline')
{
    ...
    if (GETPOST('search_idprodfournprice') || GETPOST('idprodfournprice'))	// With combolist idprodfournprice is > 0 or -1, with autocomplete, idprodfournprice is > 0 or ''
    {
        ...
        if ($idprod > 0)
        {
			...
			//FEDE
			$rate=0;
			$resql=$db->query("SELECT rate FROM llx_currency_conversion a join llx_product b on a.source=b.fk_currency join llx_facture_fourn c on a.target=c.fk_currency  where  b.rowid=".$idprod." and c.rowid=".$object->id." order by date desc");
            ...
            $result=$object->addline($label, $product->fourn_pu*$rate, $tvatx, $localtax1tx, $localtax2tx, $_POST['qty'], $idprod, $remise_percent);
...
else
{
    if ($id > 0)
    {
        ...
        else
       {
        ...
        print '<tr><td>'.$langs->trans('AmountHT').'</td><td align="right">'.price($object->total_ht).'</td><td colspan="2" align="left">'.$langs->trans('Currency'.$object->fk_currency).'</td></tr>';
        print '<tr><td>'.$langs->trans('AmountVAT').'</td><td align="right">'.price($object->total_tva).'</td><td colspan="2" align="left">'.$langs->trans('Currency'.$object->fk_currency).'</td></tr>';
        ...
        else
       {
        ...
        print '<tr><td>'.$langs->trans('AmountTTC').'</td><td align="right">'.price($object->total_ttc).'</td><td colspan="2" align="left">'.$langs->trans('Currency'.$object->fk_currency).'</td></tr>';
