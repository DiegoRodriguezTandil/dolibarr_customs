...
class Expedition extends CommonObject
{
	...
	protected $ismultientitymanaged = 1;	// 0=No test on entity, 1=Test with field entity, 2=Test with link by 
	...
	function create($user)
	{
		...
		$sql.= ", '".$this->db->escape($this->transportista)."'";
		//
	 ...
	 */
	function fetch($id, $ref='', $ref_ext='', $ref_int='',$from=0)
	{
		global $conf;
		...
		$sql.= ", e.fk_expedition_methode, e.transportista, e.tracking_number";
		...
		if ($result)
		{
			if ($this->db->num_rows($result))
			{
				...
				$this->transportista        = $obj->transportista; //transportista
				...
				//else
				//FEDE
					//$result=$this->fetch_lines_from_salesorder();				
					//$result=$this->fetch_lines_from_commande();
	...
	function fetch_lines_from_commande()
	{
		// TODO: recuperer les champs du document associe a part
		...
		$sql.= ", cd.total_ht, cd.total_localtax1, cd.total_localtax2, cd.total_ttc, cd.total_tva";
		$sql.= ", cd.tva_tx, cd.localtax1_tx, cd.localtax2_tx, cd.price, cd.subprice, cd.remise_percent";
		$sql.= ", cd.qty as qty_shipped";
		$sql.= ", p.ref as product_ref, p.label as product_label, p.fk_product_type";
		$sql.= ", p.weight, p.weight_units, p.length, p.length_units, p.surface, p.surface_units, p.volume, p.volume_units";
		$sql.= " from llx_element_element a join ";		
		$sql.= " llx_element_element b on a.fk_source=b.fk_target";	
		$sql.= " join llx_commandedet cd on b.fk_source=cd.fk_commande";
		$sql.= " join llx_product p on cd.fk_product=p.rowid";
		$sql.= " where a.fk_target=".$this->id." and a.targettype='shipping'";
	...
	function fetch_lines_from_salesorder()
	{
		// TODO: recuperer les champs du document associe a part
		...
		$sql.= ", p.weight, p.weight_units, p.length, p.length_units, p.surface, p.surface_units, p.volume, p.volume_units";
		$sql.= " from llx_element_element a join ";		
		$sql.= " llx_salesorderdet cd on a.fk_source=cd.fk_salesorder";
		$sql.= " join llx_product p on cd.fk_product=p.rowid";
		$sql.= " where a.fk_target=".$this->id." and a.targettype='shipping'";
