...
/**
 *	\file       htdocs/salesorder/document.php
 *	\ingroup    order
 *	\brief      Page de gestion des documents attachees a une salesorder
 */
...
if (GETPOST('sendit') && ! empty($conf->global->MAIN_UPLOAD_DOC))
{
	if ($object->fetch($id))
    {
	    ...
	    $upload_dir = $conf->salesorder->dir_output . "/" . dol_sanitizeFileName($object->ref);
...
if ($id > 0 || ! empty($ref))
{
	if ($object->fetch($id, $ref))
    {
		...
		$formfile->form_attach_new_file(DOL_URL_ROOT.'/salesorder/document.php?id='.$object->id,'',0,0,$user->rights->salesorder->creer,50,$object);


		// List of document
		$param='&id='.$object->id;
		$formfile->list_of_documents($filearray,$object,'salesorder',$param);
	}
	else
	{
		dol_print_error($db);
