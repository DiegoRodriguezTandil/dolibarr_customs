...
function print_eldy_menu($db,$atarget,$type_user)
{
	...
	//FEDE
	if (! empty($conf->salesorder->enabled)) $menuqualified++;
	//FIN FEDE
	...
	$tmpentry=array('enabled'=>$menuqualified, 'perms'=>($user->rights->societe->lire || $user->rights->societe->contact->lire), 'module'=>'propal|commande|salesorder|fournisseur|contrat|ficheinter');
	$showmode=dol_eldy_showmenu($type_user, $tmpentry, $listofmodulesforexternal);
	if ($showmode)
	{
    ...
    if ($mainmenu)
    {
        ...
        if ($mainmenu == 'commercial')
        {
            ...
            if (! empty($conf->propal->enabled))
            {
				...
				if ($leftmenu=="propals") $newmenu->add("/comm/propal/list.php?leftmenu=propals&viewstatut=2", $langs->trans("Approved"), 2, $user->rights->propale->lire);
				...
				if ($leftmenu=="propals") $newmenu->add("/comm/propal/list.php?leftmenu=propals&viewstatut=3", $langs->trans("Not Approved"), 2, $user->rights->propale->lire);
            ...
            if (! empty($conf->salesorder->enabled))
            {
                ...
                $newmenu->add("/salesorder/index.php?leftmenu=salesorders", $langs->trans("SalesOrders"), 0, $user->rights->salesorder->lire, '', $mainmenu, 'salesorders');
                $newmenu->add("/societe/societe.php?leftmenu=salesorders", $langs->trans("NewSalesOrder"), 1, $user->rights->salesorder->creer);
                $newmenu->add("/salesorder/liste.php?leftmenu=salesorders", $langs->trans("List"), 1, $user->rights->salesorder->lire);
                if ($leftmenu=="salesorders") $newmenu->add("/salesorder/liste.php?leftmenu=salesorders&viewstatut=0", $langs->trans("StatusOrderDraftShort"), 2, $user->rights->salesorder->lire);
                if ($leftmenu=="salesorders") $newmenu->add("/salesorder/liste.php?leftmenu=salesorders&viewstatut=1", $langs->trans("StatusSalesOrderValidated"), 2, $user->rights->salesorder->lire);
                if ($leftmenu=="salesorders" && ! empty($conf->expedition->enabled)) $newmenu->add("/salesorder/liste.php?leftmenu=salesorders&viewstatut=2", $langs->trans("StatusOrderOnProcessShort"), 2, $user->rights->salesorder->lire);
                if ($leftmenu=="salesorders") $newmenu->add("/salesorder/liste.php?leftmenu=salesorders&viewstatut=3", $langs->trans("StatusSalesOrderToBill"), 2, $user->rights->salesorder->lire);  // The translation key is StatusOrderToBill but it means StatusDelivered. TODO We should renamed this later
                if ($leftmenu=="salesorders") $newmenu->add("/salesorder/liste.php?leftmenu=salesorders&viewstatut=4", $langs->trans("StatusSalesOrderProcessed"), 2, $user->rights->salesorder->lire);
                if ($leftmenu=="salesorders") $newmenu->add("/salesorder/liste.php?leftmenu=salesorders&viewstatut=-1", $langs->trans("StatusSalesOrderCanceledShort"), 2, $user->rights->salesorder->lire);
                $newmenu->add("/salesorder/stats/index.php?leftmenu=salesorders", $langs->trans("Statistics"), 1, $user->rights->salesorder->lire);
            }
			//FIN FEDE
            ...
            if (! empty($conf->societe->enabled) && ! empty($conf->fournisseur->enabled))
            {
                ...
                {
                    $langs->load("Insurance Policies");
                    $newmenu->add("/polizas/policy_list.php?leftmenu=polizas", $langs->trans("Insurance Policies"),0,$user->rights->fournisseur->facture->lire, '', $mainmenu, 'suppliers_bills');
                    if (empty($user->societe_id))
                    {
                        $newmenu->add("/polizas/policy_list.php?action=create",$langs->trans("List (All)"),1,$user->rights->fournisseur->facture->creer);
                    }
					
					if (empty($user->societe_id))
                    {
                        $newmenu->add("/polizas/open_policy_list.php?action=create",$langs->trans("List (Open)"),1,$user->rights->fournisseur->facture->creer);
                    }
        ...
        if ($mainmenu == 'accountancy')
        {
            ...
            if (! empty($conf->societe->enabled) && ! empty($conf->fournisseur->enabled))
            {
                if (! empty($conf->facture->enabled))
                {
                    $langs->load("Exchange Rates");
                    $newmenu->add("/exchangerate/exchangeratelist.php?leftmenu=exchange_rates", $langs->trans("Exchange Rates"),0,$user->rights->fournisseur->facture->lire, '', $mainmenu, 'suppliers_bills');
                    if (empty($user->societe_id))
                    {
                        //$newmenu->add("/fourn/facture/fiche.php?action=create",$langs->trans("NewBill"),1,$user->rights->fournisseur->facture->creer);
						$newmenu->add("/exchangerate/addnewrate.php?action=create",$langs->trans("Add New Rate"),1,$user->rights->fournisseur->facture->creer);
                    }
                    $newmenu->add("/exchangerate/exchangeratelist.php", $langs->trans("List"),1,$user->rights->fournisseur->facture->lire);
