...
$result = restrictedArea($user, 'commande', $id, 'propal');
...
if(!$object->policy->fetch_by_doc(GETPOST('id'),'CO'))
{	
	$object->policy->fk_docid=GETPOST('id');
	$object->policy->fk_doctype='CO';
...
if ($id > 0 || ! empty($ref))
{
	...
	dol_fiche_head($head, 'poliza', $langs->trans("CustomerOrder"), 0, 'order');
