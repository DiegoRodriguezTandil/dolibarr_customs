...
class Commande extends CommonOrder
{
    ...
    function create($user, $notrigger=0)
    {
        ...
        $sql.= " ref, purpose, tender_number, office, fk_soc, date_creation, fk_user_author, fk_projet, fk_currency, date_commande, source, note, note_public, ref_client, ref_int";
        ...
        //$sql.= " VALUES ('(PROV)',".$this->socid.", ".$this->db->idate($now).", ".$user->id;
		 $sql.= " VALUES ('(PROV)','".$this->purpose."','".$this->tender_number."',".$this->office.", ".$this->socid.", ".$this->db->idate($now).", ".$user->id;
		//FIN FEDE
    ...
    function fetch($id, $ref='', $ref_ext='', $ref_int='')
    {
        ...
        $sql = 'SELECT c.rowid, c.date_creation, c.ref, c.office, c.fk_soc, c.fk_user_author, c.fk_statut';
    ...
    function fetch_lines($only_product=0)
    {
        ...
        $sql = 'SELECT l.rowid, l.fk_product, l.fk_parent_line, l.product_type, l.fk_commande, l.label as custom_label, l.description,l.fk_currency, l.origin_price, l.price, l.qty, l.tva_tx,';
    ...
    function setShipmentLocation($location)
    {
		...
		dol_syslog("Commande::setShipmentLocation sql=$sql",LOG_DEBUG);
		$resql=$this->db->query($sql);
		if ($resql)
		{
		...
		{
			$this->error=$this->db->error();
			dol_syslog("Commande::setShipment_location ".$this->error,LOG_ERR);
			return -1;
		}
    }
	function setPurpose($purpose)
    {
		...
		dol_syslog("Commande::setPurpose sql=$sql",LOG_DEBUG);
		$resql=$this->db->query($sql);
		if ($resql)
		{
		...
		{
			$this->error=$this->db->error();
			dol_syslog("Commande::setPurpose ".$this->error,LOG_ERR);
			return -1;
		}
    }
	function setTenderNumber($number)
    {
		...
		dol_syslog("Commande::setTenderNumber sql=$sql",LOG_DEBUG);
		$resql=$this->db->query($sql);
		if ($resql)
		{
		...
		{
			$this->error=$this->db->error();
			dol_syslog("Commande::setTenderNumber ".$this->error,LOG_ERR);
			return -1;
		}
    }
	//FIN FEDE
...
class OrderLine
{
    ...
    function insert($notrigger=0)
    {
		...
		//FEDE 16-10-2013
		$sql.= "(select price from llx_product_price where rowid=(select max(rowid) from llx_product_price where fk_product=".$this->fk_product." and price_level=(SELECT ifnull(a.price_level,1) FROM llx_societe a join llx_commande b on a.rowid=b.fk_soc where b.rowid=".$this->fk_commande."))),";		
		//FIN FEDE
