...
//CERRAR VENTANA DE ORDEN DE VENTA
...
//FIN CERRAR VENTA DE ORDEN DE VENTA
...
if ($action == 'send' && ! GETPOST('addfile') && ! GETPOST('removedfile') && ! GETPOST('cancel'))
{
			...
			*/
			if ($object->statut == 0 && $user->rights->salesorder->creer)
			{
				if ($action != 'editline')
				{
					...
					function open_win()
					{
						window.open("../product/fiche2.php?leftmenu=product&action=create&type=0", "newwindow", "height=800, width=800, modal=yes, alwaysRaised=yes, toolbar=no, menubar=no, scrollbars=no, resizable=no")
					}
					</script>
					</head>
					<body>
						<input type="button" value=" Nuevo Producto " onclick="open_win()">
					</body>';
					//FIN FEDE
			...
			*/
			if ($action != 'presend')
			{
				if ($user->societe_id == 0 && $action <> 'editline')
				{
					...
					{
						print '<a class="butAction" href="fiche2.php?id='.$object->id.'&amp;action=modif">'.$langs->trans('Modify').'</a>';
					}
  ...
  print '<a class="butAction" href="'.DOL_URL_ROOT.'/fichinter/fiche2.php?action=create&amp;origin='.$object->element.'&amp;originid='.$object->id.'&amp;socid='.$object->socid.'">'.$langs->trans('AddIntervention').'</a>';
                              }
                              else
                              {
