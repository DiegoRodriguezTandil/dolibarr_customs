...
class Policy extends CommonObject
{
	...
	/*
		...
		*/
		...
		if ($ref) $sql.= " WHERE p.expiration_date='".$this->db->escape($fk_docid)."'";
		else $sql.= " WHERE p.expiration_date=".$fk_docid;
	...
	function getNomUrl($withpicto=0)
	{
		...
		//$lien = '<a href="'.DOL_URL_ROOT.'/exchangerate/delete.php?id='.$this->date.'"> ';
		$doctype= $this->fk_doctype;
		
		if ($doctype == 'CO') {
			
		
		$lien = '<a href="'.DOL_URL_ROOT.'/commande/poliza.php?id='.$this->fk_docid.'"> ';
		}
		
		if ($doctype == 'SO') {
			
		
		$lien = '<a href="'.DOL_URL_ROOT.'/salesorder/poliza.php?id='.$this->fk_docid.'"> ';
		}
		
		if ($doctype == 'PO') {
			
		
		$lien = '<a href="'.DOL_URL_ROOT.'/fourn/commande/poliza.php?id='.$this->fk_docid.'"> ';
		}
		...
		$lien = '<a href="'.DOL_URL_ROOT.'/comm/propal/poliza.php?id='.$this->fk_docid.'"> ';
		}
		...
		//$label=$langs->trans("Show").': '.$this->date;
		//$label=$langs->trans("Delete").': '.$this->date;
		$label=$langs->trans("Ver más +");
			

//		if ($withpicto) $result.=($lien.img_object($label,$picto).$lienfin);
        if ($withpicto) $result.=($lien.($label).$lienfin);
		if ($withpicto && $withpicto != 2) $result.=' ';
		if ($withpicto != 2) $result.=$lien.$this->ref.$lienfin;
		return $result;
	}
