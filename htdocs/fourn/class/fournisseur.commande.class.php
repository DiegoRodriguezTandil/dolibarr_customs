...
class CommandeFournisseur extends CommonOrder
{
    ...
    function fetch($id,$ref='')
    {
        ...
        $sql = "SELECT c.rowid, c.ref, c.fk_shipto, ref_supplier, c.fk_soc, c.fk_statut, c.amount_ht, c.total_ht, c.total_ttc, c.tva,";
        $sql.= " c.localtax1, c.localtax2, ";
        $sql.= " c.date_creation, c.date_valid, c.date_approve,";
        $sql.= " c.fk_user_author, c.fk_user_valid, c.fk_user_approve,";
        $sql.= " c.date_commande as date_commande, c.date_livraison as date_livraison, c.fk_cond_reglement, c.fk_mode_reglement, c.fk_projet as fk_project, c.fk_currency, c.remise_percent, c.source, c.fk_input_method,";
        ...
        if ($resql)
        {
			...
			if($this->fk_shipto>0)
			{
				...
				$this->entrepot->fetch($this->fk_shipto);
			}
            ...
            if ($fk_product > 0)
            {
                ...
                if ($prod->fetch($fk_product) > 0)
                {
                    ...
                    if ($result > 0)
                    {
						...
						//FEDE
						$rate=0;
						$resql=$this->db->query("SELECT rate FROM llx_currency_conversion a join llx_product b on a.source=b.fk_currency join llx_commande_fournisseur c on a.target=c.fk_currency  where  b.rowid=".$fk_product." and c.rowid=".$this->id." order by date desc");
						if($conv = $this->db->fetch_object($resql))
						{
							$rate=$conv->rate;
						}
						//FIN FEDE
