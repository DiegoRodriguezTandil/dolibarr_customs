...
if ($action == 'add')
{
	...
	$object->transportista		= GETPOST('transportista','alpha');
	//fin agregar transportista	
...
else if ($action == 'builddoc')	// En get ou en post
{
    ...
    $shipment->fetch($id,'','','',GETPOST('source','alpha'));
	...
	//FEDE
	//
    $result=expedition_pdf_create($db,$shipment,GETPOST('model','alpha'),$outputlangs,1);
    if ($result <= 0)
    {
        dol_print_error($db,$result);
        exit;
    ...
    if ($origin)
    {
        ...
        if ($object->fetch($origin_id))	// This include the fetch_lines
        {
            ...
            */
			...
			if ($origin == 'salesorder' && ! empty($conf->commande->enabled))
            {
                print $langs->trans("RefSalesOrder").'</td><td colspan="3"><a href="'.DOL_URL_ROOT.'/salesorder/fiche.php?id='.$object->id.'">'.img_object($langs->trans("ShowOrder"),'salesorder').' '.$object->ref;
            }
			...
			//Agregar campo transportista
			//Obtener ID Transportista
			
			$resqltr=$db->query("SELECT id FROM llx_c_typent WHERE code='TE_CARRIER'");
			if ($resqltr)
				{
					$numtr = $db->num_rows($resqltr);
					$i = 0;
					if ($numtr)
						{
							while ($i < $numtr)
								{
									$obj3 = $db->fetch_object($resqltr);
									if ($obj3)
									{
									$id_transportista= $obj3->id;						
									}
									$i++;
								}
						}
			...
			//fin obtener ID Transportista
			//Mostrar combo con transportistas que estan en DB
			 print "<tr><td>";
			 //agregar en diccionario y traducción
			 print "Transportista</td>";
			 print "<td>";
			 print '<select name="transportista">';
			 print '<option> </option>';
			
			//obtener transportistas
			
			 $resql=$db->query("SELECT nom FROM llx_societe WHERE fk_typent= '$id_transportista'");
				if ($resql)
					{
			...
			//fin agregar campo transportista
...
/* *************************************************************************** */
{
    if (! empty($id) || ! empty($ref))
    {
        ...
        if ($object->id > 0)
        {
			...
			if ($typeobject == 'salesorder' && $object->$typeobject->id && ! empty($conf->salesorder->enabled))
            {
                print '<tr><td>';
                $objectsrc=new Salesorder($db);
                $objectsrc->fetch($object->$typeobject->id);
                print $langs->trans("RefSalesOrder").'</td>';
                print '<td colspan="3">';
                print $objectsrc->getNomUrl(1,'salesorder');
                print "</td>\n";
                print '</tr>';
            }
			...
			//Transportista
			print '<tr><td>'.$form->editfieldkey("Transportista",'transportista',$object->transportista,$object,$user->rights->expedition->creer).'</td><td colspan="3">';
            print $form->editfieldval("Transportista",'transportista',$object->transportista,$object,$user->rights->expedition->creer,'string',$object->transportista);
            print '</td></tr>';
