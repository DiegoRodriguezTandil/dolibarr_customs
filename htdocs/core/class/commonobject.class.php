...
abstract class CommonObject
{
	...
	//FEDE	
	function delete_deplacement($rowid, $notrigger=0)
    {
        global $user,$langs,$conf;

		$error=0;

        $sql = "DELETE FROM ".MAIN_DB_PREFIX."deplacement";
        $sql.= " WHERE rowid =".$rowid;
        ...
        if ($this->db->query($sql))
        {
            if (! $notrigger)
            {
                ...
                include_once DOL_DOCUMENT_ROOT . '/core/class/interfaces.class.php';
                $interface=new Interfaces($this->db);
                $result=$interface->run_triggers(strtoupper($this->element).'_DELETE_DEPLACEMENT',$this,$user,$langs,$conf);
                if ($result < 0) {
                    $error++; $this->errors=$interface->errors;
                }
                // End call triggers
	...
	function setCurrency($currencyid)
    {
        ...
        $sql = 'UPDATE '.MAIN_DB_PREFIX.$this->table_element;
        if ($currencyid) $sql.= " SET fk_currency = '".$currencyid."'";
        else $sql.= " SET fk_currency = NULL";
        $sql.= ' WHERE rowid = '.$this->id;

        dol_syslog(get_class($this)."::setCurrency sql=".$sql);
        if ($this->db->query($sql))
        {
            $this->fk_currency = $currencyid;
            return 1;
        }
        else
        {
	...
	function setShipto($shiptoid)
    {
        if (! $this->table_element)
        {
            dol_syslog(get_class($this)."::setShipto was called on objet with property table_element not defined",LOG_ERR);
            return -1;
        }

        $sql = 'UPDATE '.MAIN_DB_PREFIX.$this->table_element;
        if ($shiptoid) $sql.= " SET fk_shipto = ".$shiptoid;
        else $sql.= " SET fk_shipto = NULL";
        $sql.= ' WHERE rowid = '.$this->id;

        dol_syslog(get_class($this)."::setShipto sql=".$sql);
        if ($this->db->query($sql))
        {
            $this->fk_shipto = $shiptoid;
            return 1;
        }
        else
        {
	...
	function setWarehouse($shiptoid)
    {
        if (! $this->table_element)
        {
            dol_syslog(get_class($this)."::setShipto was called on objet with property table_element not defined",LOG_ERR);
        ...
        $sql = 'UPDATE '.MAIN_DB_PREFIX.$this->table_element;
        if ($shiptoid) $sql.= " SET fk_entrepot = ".$shiptoid;
        else $sql.= " SET fk_entrepot = NULL";
        $sql.= ' WHERE rowid = '.$this->id;

        dol_syslog(get_class($this)."::setShipto sql=".$sql);
        if ($this->db->query($sql))
        {
            $this->fk_entrepot = $shiptoid;
            return 1;
        }
        else
        {
    ...
    function getElementUrl($objectid,$objecttype,$withpicto=0,$option='')
    {
		...
		if ($objecttype == 'policy' ) {
            $classpath = 'core/class'; $module='policy'; $subelement='policy';
        }
        ...
        if (empty($reshook))
        {
        	...
        	foreach($this->linkedObjects as $objecttype => $objects)
        	{
        		...
        		if ($objecttype == 'facture')          {
        			...
        			$tplpath = 'salesorder';
        			if (empty($conf->salesorder->enabled)) continue;	// Do not show if module disabled
        		}
        		else if ($objecttype == 'shipping')         {
        			$tplpath = 'expedition';
	...
	function printObjectLines($action, $seller, $buyer, $selected=0, $dateSelector=0, $hookmanager=false)
	{
		...
		print '<td align="right" width="50">'.$langs->trans('Precio Orig.').'</td>';
		print '<td align="center" width="50">'.$langs->trans('Factor').'</td>';
		//FIN FEDE	
