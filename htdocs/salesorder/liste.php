...
/**
 *	\file       htdocs/salesorder/liste.php
 *	\ingroup    salesorder
...
$sql.= ' c.date_valid, c.date_salesorder, c.date_livraison, c.fk_statut, c.facture as facturee';
...
if ($search_user > 0)
{
    $sql.= " AND ec.fk_c_type_contact = tc.rowid AND tc.element='salesorder' AND tc.source='internal' AND ec.element_id = c.rowid AND ec.fk_socpeople = ".$search_user;
}
...
if ($resql)
{
	if ($socid)
	{
		$soc = new Societe($db);
		$soc->fetch($socid);
		$title = $langs->trans('ListOfSalesOrders') . ' - '.$soc->nom;
	}
	else
	{
		$title = $langs->trans('ListOfSalesOrders');
	}
	...
	print_liste_field_titre($langs->trans('RefCustomerSalesOrder'),$_SERVER["PHP_SELF"],'c.ref_client','',$param,'',$sortfield,$sortorder);
	print_liste_field_titre($langs->trans('SalesOrderDate'),$_SERVER["PHP_SELF"],'c.date_salesorder','',$param, 'align="right"',$sortfield,$sortorder);
	...
	$generic_salesorder = new Salesorder($db);
	while ($i < min($num,$limit))
	{
		...
		$generic_salesorder->id=$objp->rowid;
		$generic_salesorder->ref=$objp->ref;

		print '<table class="nobordernopadding"><tr class="nocellnopadd">';
		print '<td class="nobordernopadding" nowrap="nowrap">';
		print $generic_salesorder->getNomUrl(1,($viewstatut != 2?0:$objp->fk_statut));
		print '</td>';

		print '<td width="20" class="nobordernopadding" nowrap="nowrap">';
		if (($objp->fk_statut > 0) && ($objp->fk_statut < 3) && $db->jdate($objp->date_valid) < ($now - $conf->salesorder->client->warning_delay)) print img_picto($langs->trans("Late"),"warning");
		print '</td>';

		print '<td width="16" align="right" class="nobordernopadding">';
		$filename=dol_sanitizeFileName($objp->ref);
		$filedir=$conf->salesorder->dir_output . '/' . dol_sanitizeFileName($objp->ref);
		$urlsource=$_SERVER['PHP_SELF'].'?id='.$objp->rowid;
		print $formfile->getDocumentsLink($generic_salesorder->element, $filename, $filedir);
		print '</td></tr></table>';
		...
		print '&nbsp;<a href="'.DOL_URL_ROOT.'/salesorder/orderstoinvoice.php?socid='.$companystatic->id.'">';
		...
		// Order date
		$y = dol_print_date($db->jdate($objp->date_salesorder),'%Y');
		$m = dol_print_date($db->jdate($objp->date_salesorder),'%m');
		$ml = dol_print_date($db->jdate($objp->date_salesorder),'%B');
		$d = dol_print_date($db->jdate($objp->date_salesorder),'%d');
		...
		// Statut
		print '<td align="right" nowrap="nowrap">'.$generic_salesorder->LibStatut($objp->fk_statut,$objp->facturee,5).'</td>';
