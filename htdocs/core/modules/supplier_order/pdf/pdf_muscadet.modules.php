...
class pdf_muscadet extends ModelePDFSuppliersOrders
{
		...
		if ($conf->fournisseur->dir_output.'/commande')
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0 ; $i < $nblignes ; $i++)
				{
					...
					//FEDE se concatenó la unidad de medida
					$pdf->MultiCell($this->posxdiscount-$this->posxqty-1, 3, $object->lines[$i]->qty.' '.$langs->trans(measuring_units_string($object->lines[$i]->measure_unit,"measure")), 0, 'R');
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager)
	{
		...
		//FEDE Referencia proveedor
		$posy+=5;
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		if (isset($object->ref_supplier)) {
		$pdf->MultiCell(100, 4, 'Ref' . ' ' . $outputlangs->convToOutputCharset($object->ref_supplier), '', 'R');
		} 
		...
		//FEDE
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 3, $outputlangs->transnoentities("OrderDate")." : " . dol_print_date($object->date_valid,"day",false,$outputlangs,true), '', 'R');
		//FIN FEDE
		...
		if ($showaddress)
		{
			...
			$pdf->MultiCell(66,5, $outputlangs->transnoentities("Ship To").":", 0, 'L');
			$pdf->SetXY($posx,$posy);
			...
			//$pdf->MultiCell(80, 4, $outputlangs->convToOutputCharset($this->entrepot->name), 0, 'L');
			$pdf->MultiCell(80, 4, $outputlangs->convToOutputCharset($object->entrepot->libelle), 0, 'L');

			// Show sender information
			//$pdf->SetXY($posx+2,$posy+4+(dol_nboflines_bis($this->emetteur->name,44)*4));
			$pdf->SetXY($posx+2,$posy+4+(dol_nboflines_bis($object->entrepot->address,44)*4));
			$pdf->SetFont('','', $default_font_size - 1);
			
			// address
			$stringaddress .= ($stringaddress ? "\n" : '' ).$outputlangs->convToOutputCharset($object->entrepot->address);
			// ville
			$stringaddress .= ($stringaddress ? "\n" : '' ).$outputlangs->convToOutputCharset($object->entrepot->ville);	
			// cp
			$stringaddress .= ', '.$outputlangs->convToOutputCharset($object->entrepot->cp);
			$stringaddress .= ($stringaddress ? "\n" : '' ).$outputlangs->convToOutputCharset($outputlangs->transnoentitiesnoconv("Country".$object->entrepot->pays_code));
			
			// phone
			//$stringaddress .= ($stringaddress ? "\nPhone: " : '' ).$outputlangs->convToOutputCharset($object->entrepot->phone_pro);
			$stringaddress .= ($stringaddress ? "\n" : '').$outputlangs->transnoentities("Phone"). ":";
			$stringaddress .= ($stringaddress ? "" : '' ).$outputlangs->convToOutputCharset($object->entrepot->phone_pro);
			// phone
			$stringaddress .= ($stringaddress ? "\nFax: " : '' ).$outputlangs->convToOutputCharset($object->entrepot->fax);
			// EMAIL
			$stringaddress .= ($stringaddress ? "\nEmail: " : '' ).$outputlangs->convToOutputCharset($object->entrepot->email);
			
			$pdf->MultiCell(80, 4,$stringaddress, 0, 'L');
			...
			$pdf->MultiCell($widthrecbox, 5, $outputlangs->transnoentities("Vendor").":",0,'L');
			$pdf->MultiCell($widthrecbox, 5, $outputlangs->transnoentities("")." ",0,'L');
			...
			//Mostrar info de destino (deposito
			$pdf->SetFont('','', $default_font_size - 1);
			$pdf->SetXY($posx-89,$posy+43);		
			
			//Mostrar los datos del deposito
			/*		
			$pdf->SetTextColor(0,0,0);
			$pdf->SetFont('','', $default_font_size - 2);
			//$pdf->SetXY($posx-89,$posy+50);
			//$pdf->MultiCell(66,5, $outputlangs->transnoentities("Test").":", 0, 'L');
			$pdf->SetXY($posx-90,$posy+42);
			$pdf->MultiCell($widthrecbox, 4,$outputlangs->transnoentities("Ship To").":" , '', 'L');//Enviar a:
			//$pdf->SetFillColor(230,230,230); //COLOR DE FONDO RECTANGULO
			//$pdf->MultiCell(82, $hautcadre=30, "", 0, 'R', 1);//DIMENSIONES DEL RECTANGULO
			//$pdf->SetTextColor(0,0,60);
			$pdf->Rect($posx-90, $posy+48, $widthrecbox=82, $hautcadre=20); //RECTANGULO CON BORDERS Y FONDO BLANCO $hautcadre setea la altura
			
			
			
			//end test frame
			//$pdf->MultiCell($widthrecbox, 4,$outputlangs->transnoentities("BillTo").":" , '', 'L');
			
			$pdf->SetXY($posx-89,$posy+50);	
			//$pdf->SetFillColor(230,230,230);
			//$pdf->SetFillColor(245,27,27);
			$pdf->SetFont('','', $default_font_size - 1);
			$pdf->MultiCell($widthrecbox, 4, ""."" . $outputlangs->convToOutputCharset($object->entrepot->description), '', 'L');
			$pdf->MultiCell($widthrecbox, 4, " "."" . $outputlangs->convToOutputCharset($object->entrepot->cp) . " " .  $outputlangs->convToOutputCharset($object->entrepot->ville), '', 'L'); //IMPRIME CODIGO POSTAL Y CIUDAD
		    //$pdf->MultiCell($widthrecbox, 4, " "."" . $outputlangs->convToOutputCharset($object->entrepot->ville), '', 'L');
			//$pdf->Rect($posx, $posy, $widthrecbox, $hautcadre);
			*/
		}
