...
class pdf_expedition_rouget extends ModelePdfExpedition
{
		...
		if ($conf->expedition->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				if (! empty($object->note_public) || ! empty($object->tracking_number))
				{
					...
					if (! empty($object->tracking_number))
					{
						...
						if (! empty($object->tracking_url))
						{
							if ($object->expedition_method_id > 0)
							{
								...
								$label=$outputlangs->trans("LinkToTrackYourPackage").":"." ". strtoupper($object->tracking_url);
								//$pdf->SetFont('','B', $default_font_size - 2);
								$pdf->writeHTMLCell(100, 4, $this->posxdesc-1, $tab_top-1, $label, 0, 1, false, true, 'L');
								//$label.=$outputlangs->trans("SendingMethod".strtoupper($code))." :";
								$label2.="Metodo de Envio".":"." ".strtoupper($code);
								//$pdf->SetFont('','B', $default_font_size - 2);
								$pdf->writeHTMLCell(100, 4, $this->posxdesc-1, 98, $label2, 0, 1, false, true, 'L');
								//$pdf->SetFont('','B', $default_font_size - 2);
					...
					// Transportista
					if (! empty($object->transportista))
					{
						$pdf->SetFont('','', $default_font_size - 1);   // Dans boucle pour gerer multi-page
						$pdf->writeHTMLCell(100, 30, $this->posxdesc-1, 108, dol_htmlentitiesbr($object->transportista), 0, 1);
					}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if ($this->emetteur->logo)
		{
			if (is_readable($logo))
			{
				...
				//mostrar el logo de acuerdo a la oficina de ventas
				
				//conectarse a db y obtener el nombre de la oficina
				//
				...
				//
				  $officeid= $linkedobject->office;
				  require_once DOL_DOCUMENT_ROOT.'/exchangerate/conexiondb.php';
				  conectar_db();
				  $queryoffice= mysql_query("SELECT nom FROM llx_societe WHERE rowid='$officeid'");
				  if (isset($queryoffice)) {
				  while ($row = mysql_fetch_array($queryoffice)) 
				  {
			      $officename= $row['nom'];
				   }
											}
				...
				//fin mostrar el logo de acuerdo a la oficina de ventas
			}
			else
			{
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		//sacar leyenda "referencia envío" del pdf
			...
			if ($result >= 0)
			{
				...
				if ($origin=='commande')
					$pdf->MultiCell(100, 2, $outputlangs->transnoentities("RefOrder") ." : ".$outputlangs->transnoentities($text), 0, 'R');
					 ...
					 //sacar referencia de orden de venta del PDF y mostrar el proyecto al que está asociado
					//$pdf->MultiCell(100, 2, $outputlangs->transnoentities("RefSalesOrder") ." : ".$outputlangs->transnoentities($text), 0, 'R');
					  //obtener la referencia del proyecto asociado	
			      $rowid= $linkedobject->fk_project;
				  require_once DOL_DOCUMENT_ROOT.'/exchangerate/conexiondb.php';
				  conectar_db();
				  $query= mysql_query("SELECT ref FROM llx_projet WHERE rowid='$rowid'");
				  if (isset($query)) {
				  ...
				  $pdf->MultiCell(100, 2, $outputlangs->transnoentities($projref), 0, 'R');
				  //fin obtener referencia del proyecto asociado
	...
	{
	   //Mostrar footer solo para oficina de ventas Redimec SRL
