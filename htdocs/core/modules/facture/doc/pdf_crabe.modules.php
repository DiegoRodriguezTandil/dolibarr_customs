...
class pdf_crabe extends ModelePDFFactures
{
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		//setear tab_top para la línea de ingresos brutos
		/*
		$tab2_top = $tab2_top + 4;
		//
		$pdf->SetFillColor(255,255,255);
		$pdf->SetXY($col1x, $tab2_top + 0);
		$pdf->MultiCell($col2x-$col1x, $tab2_hl, $outputlangs->transnoentities("Retención Ingresos Brutos"), 0, 'L', 1);
		$pdf->SetXY($col2x, $tab2_top + 0);
		//$pdf->MultiCell($largcol2, $tab2_hl, price($sign * ($object->ingresosbrutos + (! empty($object->remise)?$object->remise:0))), 0, 'R', 1);
					
		//$pdf->MultiCell($largcol2, $tab2_hl, number_format((float)$object->ingresosbrutos, 2, ',', ''), 0, 'R', 1);
		$pdf->MultiCell($largcol2, $tab2_hl, price($object->ingresosbrutos), 0, 'R', 1);
		//$pdf->MultiCell($largcol2, $tab2_hl, $parsedingbrutos, 0, 'R', 1);
		*/
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager)
	{
		...
		if ($this->emetteur->logo)
		{
			if (is_readable($logo))
			{
				...
				// fin mostrar logo de acuerdo a la empresa
		...
		if ($showaddress)
		{
			...
			$pdf->SetFont('','', $default_font_size  - 1);
			$pdf->MultiCell(80, 4, $officelogo->country, 0, 'L');
			//
