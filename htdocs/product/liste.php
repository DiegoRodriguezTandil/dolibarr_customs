...
else
{
    ...
    $sql = 'SELECT DISTINCT p.rowid, p.ref, p.label,p.description, p.barcode, p.price, p.price_ttc, p.price_base_type,';
    ...
    if ($resql)
    {
    	...
    	else
    	{
			...
			print_liste_field_titre($langs->trans("Part Number/ Serial Number"), $_SERVER["PHP_SELF"], "p.ref",$param,"","",$sortfield,$sortorder);
    		print_liste_field_titre($langs->trans("Código"), $_SERVER["PHP_SELF"], "p.label",$param,"","",$sortfield,$sortorder);
			}
			else
			{
			print_liste_field_titre($langs->trans("Part Number"), $_SERVER["PHP_SELF"], "p.ref",$param,"","",$sortfield,$sortorder);
    		print_liste_field_titre($langs->trans("Serial Number"), $_SERVER["PHP_SELF"], "p.label",$param,"","",$sortfield,$sortorder);
			
			}
			
			print_liste_field_titre($langs->trans("Description"), $_SERVER["PHP_SELF"], "p.ref",$param,"","",$sortfield,$sortorder);
    		...
    		while ($i < min($num,$limit))
    		{
    			...
    			//print '<td>'.dol_trunc($objp->description, 20).'</td>';
						
				print '<td width="20%">'.substr($objp->description, 0, 40) . '...' .'</td>';
