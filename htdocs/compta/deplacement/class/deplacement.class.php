...
class Deplacement extends CommonObject
{
	...
	*/
	...
	function update($user)
	{
		...
		//FEDE
		$sql .= " , fk_currency = '".price2num($this->fk_currency)."'";
	...
	*/
	function fetch($id)
	{
		$sql = "SELECT rowid, fk_user, type, fk_statut, km, fk_currency,total_ht, fk_soc, dated, note, note_public, fk_projet, extraparams, fk_doctype, fk_docid";
	...
	{
		$sql = "SELECT rate FROM llx_currency_conversion a join llx_deplacement b on a.source=b.fk_currency and a.target='".$currency."' and a.date<=b.dated where rowid=".$id." order by date desc";
	...
	*/
	 ...
	 */
	function getNomUrl($withpicto=0,$not_img=0)
	{
		global $langs;
		...
		if ($withpicto && $not_img==0) $result.=($lien.img_object($label,$picto).$lienfin);
