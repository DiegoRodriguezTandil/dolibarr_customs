...
'salesorder'=>array(
	'title'=>"Ordenes de venta asociadas al proyecto",
	'class'=>'Salesorder',
...
foreach ($listofreferent as $key => $value)
{
	...
	if ($qualified)
	{
		...
		print '<td>'.$langs->trans("Divisa").'</td>';
			...
			for ($i = 0; $i < $num; $i++)
			{
				...
				*/
				if(!array_key_exists($element->fk_currency,$arrayCurrencys)){
					...
					$arrayCurrencys[$element->fk_currency]['ttc']=$element->total_ttc;
				}else{
					$arrayCurrencys[$element->fk_currency]['ht']= $arrayCurrencys[$element->fk_currency]['ht']+$element->total_ht;
					$arrayCurrencys[$element->fk_currency]['ttc']= $arrayCurrencys[$element->fk_currency]['ttc']+$element->total_ttc;
				}	
				/**********************************************************************************************************************/
			...
			*/ 
			foreach ($arrayCurrencys as $divisa => $arrayTotales)	{
				...
				foreach ($arrayTotales as $tipo_total => $total)	{
					if($tipo_total==='ttc'){
						print '<td align="right" title="Importe Total"><b><I>'.$divisa.' '.price($total).'</I></b></td>';
						
					}else{
						print '<td align="right" title="Base Imponible Total"><b><I>'.$divisa.' '.price($total).'</I></b></td>';
					}
