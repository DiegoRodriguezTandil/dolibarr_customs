...
if ($action == 'edit')
{
	...
	if (! empty($product->multilangs))
	{
		foreach ($product->multilangs as $key => $value)
		{
			...
			print '<table class="border" width="100%">';
			print '<tr><td valign="top" width="15%" class="fieldrequired">'.$langs->trans('Serial Number').'</td><td><input name="libelle-'.$key.'" size="40" value="'.$product->multilangs[$key]["label"].'"></td></tr>';
...
else
{
	...
	if (! empty($product->multilangs))
	{
		foreach ($product->multilangs as $key => $value)
		{
			...
			print '<table class="border" width="100%">';
			print '<tr><td width="15%">'.$langs->trans('Serial Number').'</td><td>'.$product->multilangs[$key]["label"].'</td></tr>';
