...
/**
 *	\file       htdocs/salesorder/fiche.php
 *	\ingroup    salesorder
...
require_once DOL_DOCUMENT_ROOT.'/core/modules/salesorder/modules_salesorder.php';
...
// Action clone object
if ($action == 'confirm_clone' && $confirm == 'yes' && $user->rights->salesorder->creer)
{
	...
	else
	{
		if ($object->id > 0)
		{
			...
			if ($result > 0)
			{
				...
				exit;
                                                        //CERRAR VENTANA ORDER DE VENTA
			print'<!doctype html><html><head><script>function loaded()
			{    
			...
			<body onLoad="loaded()">Orden de Venta ingresada</body>';
			exit;											
                                                        
                                                        // FIN CERRAR VENTANA ORDER DE VENTA                
			}
			else
			{
...
else if ($action == 'reopen' && $user->rights->salesorder->creer)
{
	if ($object->statut == 3)
	{
...
// Suppression de la salesorder
else if ($action == 'confirm_delete' && $confirm == 'yes' && $user->rights->salesorder->supprimer)
{
...
else if ($action == 'confirm_deleteline' && $confirm == 'yes' && $user->rights->salesorder->creer)
{
	...
	if ($result > 0)
	{
		...
		if (empty($conf->global->MAIN_DISABLE_PDF_AUTOUPDATE))
		{
			...
			salesorder_pdf_create($db, $object, $object->modelpdf, $outputlangs, $hidedetails, $hidedesc, $hideref, $hookmanager);
		}
...
else if ($action == 'classin' && $user->rights->salesorder->creer)
{
	$object->setProject(GETPOST('projectid'));
}
// Shipment location
else if ($action == 'setshipment_location' && $user->rights->commande->creer)
{	
	$object->setShipmentLocation(GETPOST('shipment_location'));
}

// Add order
else if ($action == 'add' && $user->rights->salesorder->creer)
{
	$datesalesorder  = dol_mktime(12, 0, 0, GETPOST('remonth'),  GETPOST('reday'),  GETPOST('reyear'));
	...
	if (! $error)
	{
		...
		$object->date_salesorder        = $datesalesorder;
		...
		//FEDE 15-10-2013
		$object->fk_currency  = GETPOST('fk_currency');
		$object->office= GETPOST('office');
		//FEDE
...
else if ($action == 'classifybilled' && $user->rights->salesorder->creer)
{
	$ret=$object->classifyBilled();
}

// Positionne ref salesorder client
else if ($action == 'set_ref_client' && $user->rights->salesorder->creer)
{
	$object->set_ref_client($user, GETPOST('ref_client'));
}

else if ($action == 'setremise' && $user->rights->salesorder->creer)
{
	$object->set_remise($user, GETPOST('remise'));
}

else if ($action == 'setabsolutediscount' && $user->rights->salesorder->creer)
{
...
else if ($action == 'setdate' && $user->rights->salesorder->creer)
{
...
else if ($action == 'setdate_livraison' && $user->rights->salesorder->creer)
{
	//print "x ".$_POST['liv_month'].", ".$_POST['liv_day'].", ".$_POST['liv_year'];
...
else if ($action == 'setmode' && $user->rights->salesorder->creer)
{
	$result = $object->setPaymentMethods(GETPOST('mode_reglement_id','int'));
	if ($result < 0) dol_print_error($db,$object->error);
}

else if ($action == 'setavailability' && $user->rights->salesorder->creer)
{
	$result=$object->availability(GETPOST('availability_id'));
	if ($result < 0) dol_print_error($db,$object->error);
}

else if ($action == 'setdemandreason' && $user->rights->salesorder->creer)
{
	$result=$object->demand_reason(GETPOST('demand_reason_id'));
	if ($result < 0) dol_print_error($db,$object->error);
}

else if ($action == 'setconditions' && $user->rights->salesorder->creer)
{
...
else if ($action == 'setremisepercent' && $user->rights->salesorder->creer)
{
	$result = $object->set_remise($user, GETPOST('remise_percent'));
}

else if ($action == 'setremiseabsolue' && $user->rights->salesorder->creer)
{
	$result = $object->set_remise_absolue($user, GETPOST('remise_absolue'));
}

else if ($action == 'setnote_public' && $user->rights->salesorder->creer)
{
	$result=$object->update_note_public(dol_html_entity_decode(GETPOST('note_public'), ENT_QUOTES));
	if ($result < 0) dol_print_error($db,$object->error);
}

else if ($action == 'setnote' && $user->rights->salesorder->creer)
{
...
/*
 *  Ajout d'une ligne produit dans la salesorder
*/
else if ($action == 'addline' && $user->rights->salesorder->creer)
{
	$langs->load('errors');
	$error = false;
		...
		if (! empty($idprod))
		{
			...
			else
			{
				...
				if (! empty($conf->global->PRODUIT_MULTIPRICES) && ! empty($object->client->price_level))
				{
					//FEDE
					$rate=0;
					$resql=$db->query("SELECT rate FROM llx_currency_conversion a join llx_product b on a.source=b.fk_currency join llx_salesorder c on a.target=c.fk_currency  where  b.rowid=".$idprod." and c.rowid=".$object->id." order by date desc");
					if($conv = $db->fetch_object($resql))
					{
						$rate=$conv->rate;
					}
					
					$pu_ht = $prod->multiprices[$object->client->price_level]*$rate;
					//$pu_ht = $prod->multiprices[$object->client->price_level];
					
					$pu_ttc = $prod->multiprices_ttc[$object->client->price_level]*$rate;
					$price_min = $prod->multiprices_min[$object->client->price_level]*$rate;
					$price_base_type = $prod->multiprices_base_type[$object->client->price_level]*$rate;
					//FIN FEDE
				}
				else
				{
...
/*
 *  Mise a jour d'une ligne dans la salesorder
*/
...
else if ($action == 'updateligne' && $user->rights->salesorder->creer && GETPOST('cancel') == $langs->trans('Cancel'))
{
	header('Location: '.$_SERVER['PHP_SELF'].'?id='.$object->id);   // Pour reaffichage de la fiche en cours d'edition
	exit;
}

else if ($action == 'confirm_validate' && $confirm == 'yes' && $user->rights->salesorder->valider)
{
	...
	if (! $error)
	{
		...
		if ($result	>= 0)
		{
			...
			if (empty($conf->global->MAIN_DISABLE_PDF_AUTOUPDATE)) salesorder_pdf_create($db, $object, $object->modelpdf, $outputlangs, $hidedetails, $hidedesc, $hideref, $hookmanager);
		}
	}
}

// Go back to draft status
else if ($action == 'confirm_modif' && $user->rights->salesorder->creer)
{
	$idwarehouse=GETPOST('idwarehouse');
...
else if ($action == 'confirm_shipped' && $confirm == 'yes' && $user->rights->salesorder->cloturer)
{
	$result = $object->cloture($user);
	if ($result < 0) $mesgs=$object->errors;
}

else if ($action == 'confirm_cancel' && $confirm == 'yes' && $user->rights->salesorder->valider)
{
	$idwarehouse=GETPOST('idwarehouse');
...
else if ($action == 'up' && $user->rights->salesorder->creer)
{
...
else if ($action == 'down' && $user->rights->salesorder->creer)
{
...
else if ($action == 'builddoc')	// In get or post
{
	...
	* define into /core/modules/salesorder/modules_salesorder.php
	*/
	...
	$result=salesorder_pdf_create($db, $object, $object->modelpdf, $outputlangs, $hidedetails, $hidedesc, $hideref, $hookmanager);
...
else if ($action == 'remove_file')
{
	if ($object->id > 0)
	{
		...
		$langs->load("other");
		$upload_dir = $conf->salesorder->dir_output;
...
if ($action == 'send' && ! GETPOST('addfile') && ! GETPOST('removedfile') && ! GETPOST('cancel'))
{
	...
	if ($object->id > 0)
	{
		...
		//        $file = $conf->salesorder->dir_output . '/' . $ref . '/' . $ref . '.pdf';
	...
	if (! empty($conf->global->MAIN_DISABLE_CONTACTS_TAB) && $user->rights->salesorder->creer)
	{
	...
	*/
	...
	llxHeader('',$langs->trans('SalesOrder'),'EN:Customers_Orders|FR:salesorders_Clients|ES:Ordenes de ventas');
	...
	*
	*********************************************************************/
	if ($action == 'create' && $user->rights->salesorder->creer)
	{
		//WYSIWYG Editor
		require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';

		print_fiche_titre($langs->trans('CreateSalesOrder'));
		...
		if (! empty($origin) && ! empty($originid))
		{
			...
			{
				// For compatibility
				if ($element == 'order' || $element == 'salesorder')    {
					$element = $subelement = 'salesorder';
				}
				if ($element == 'propal')   {
					$element = 'comm/propal'; $subelement = 'propal';
		...
		print '<form name="crea_salesorder" action="'.$_SERVER["PHP_SELF"].'" method="POST">';
		...
		//FEDE
		print '<tr><td class="fieldrequired">Oficina de Ventas</td>';
		print '<td>';
		print $form->select_company(2,'office','(s.fk_typent=101)');
		print '</td></tr>';
		//FIN FEDE
		...
		*/
		...
		$form->select_date('','re','','','',"crea_salesorder",1,1);
		print '</td></tr>';
		...
		$form->select_date($datedelivery,'liv_','','','',"crea_salesorder",1,1);
		print "</td></tr>";
		...
		//FEDE 16-10-2013
		$var=!$var;
		print '<tr '.$bc[$var].'><td>'.$langs->trans("Currency").'</td><td>';
		$form->select_currency($conf->currency,'fk_currency');
		print '</td></tr>'."\n";
		//FIN FEDE
		...
		print '<td colspan="2">';
		include_once DOL_DOCUMENT_ROOT.'/core/modules/salesorder/modules_salesorder.php';
		$liste=ModelePDFsalesorders::liste_modeles($db);
		print $form->selectarray('model',$liste,$conf->global->SALESORDER_ADDON_PDF);
		print "</td></tr>";
		...
		if (! empty($origin) && ! empty($originid) && is_object($objectsrc))
		{
			...
			//FEDE 15-10.2013
			print '<input type="hidden" name="fk_currency"       value="'.$objectsrc->fk_currency.'">';
			//FEDE
			...
			//FEDE 15-10-2013
			print '<tr><td>Divisa</TD><td>'.$langs->trans('Currency'.$objectsrc->fk_currency).'</td></tr>';
			//FEDE
	...
	else
	{
		...
		if ($object->id > 0)
		{
			...
			//FEDE para crear el objeto de punto de venta y poder recuperar el nombre
			$office = new Societe($db);
			$office->fetch($object->office);
			//FIN FEDE
			...
			dol_fiche_head($head, 'order', $langs->trans("SalesOrder"), 0, 'order');

			$formconfirm='';

			/*
			 * Confirmation de la suppression de la salesorder
			*/
			if ($action == 'delete')
			{
				$formconfirm=$form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteSalesOrder'), $langs->trans('ConfirmDeleteSalesOrder'), 'confirm_delete', '', 0, 1);
			}
			...
			*/
			...
			//FEDE
			print '<tr><td>Oficina de Ventas</td><td colspan="5">';
			print $office->getNomUrl(1);
			print '</td></tr>';
			//FIN FEDE
			...
			if ($user->rights->salesorder->creer && $action == 'refcustomer')
			{
			...
			// FEDE Lugar de entrega						
			print '<tr><td height="10">';
			print '<table class="nobordernopadding" width="100%"><tr><td>';
			print 'Lugar de entrega';
			print '</td>';
			if ($action != 'editshipment_location') print '<td align="right"><a href="'.$_SERVER["PHP_SELF"].'?action=editshipment_location&amp;id='.$object->id.'">'.img_edit($langs->trans('SetDeliveryLocation'),1).'</a></td>';
			print '</tr></table>';
			print '</td><td colspan="2">';
			if ($action == 'editshipment_location')
			{
			...
			{
				print $object->shipment_location;
			}
			print '</td></tr>';
			//FIN FEDE
			...
			print '<td>'.$langs->trans('Currency'.$object->fk_currency).'</td>';
			//FIN FEDE
			...
			print '<td>'.$langs->trans('Currency'.$object->fk_currency).'</td></tr>';
			//FIN FEDE
			...
			*/
			if ($object->statut == 0 && $user->rights->salesorder->creer)
			{
				if ($action != 'editline')
				{
					$var=true;
			...
			*/
			if ($action != 'presend')
			{
				if ($user->societe_id == 0 && $action <> 'editline')
				{
					...
					function open_win()
					{
						...
						window.open(".././product/fiche2.php?leftmenu=product&action=create&type=0&"+ "pricelevel=" + prlevel, "newwindow", "height=700, width=700, modal=yes, alwaysRaised=yes, toolbar=no, menubar=no, scrollbars=no, resizable=no")
					}
					</script>
					<input type="hidden" value="<?php echo $pricelevel ?>" name="pricelevel" id="pricelevel"/>
					<?php
					...
					// Valid
					if ($object->statut == 0 && $object->total_ttc >= 0 && $numlines > 0 && $user->rights->salesorder->valider)
					{
						print '<a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;action=validate">'.$langs->trans('Validate').'</a>';
					}

					// Edit
					if ($object->statut == 1 && $user->rights->salesorder->creer)
					{
					...
					if ($object->statut > 0)
					{
						if ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) || $user->rights->salesorder->order_advance->send))
						{
					...
					if (! empty($conf->expedition->enabled))
					{
						...
						if ($object->statut > 0 && $object->statut < 3 && $object->getNbOfProductsLines() > 0)
						{
									...
									|| ($conf->livraison_bon->enabled && $user->rights->expedition->livraison->creer))
							{
								...
								{
									print '<a class="butAction" href="'.DOL_URL_ROOT.'/expedition/shipment2.php?id='.$object->id.'">'.$langs->trans('ShipProduct').'</a>';
								}
								else
								{
					...
					if (! empty($conf->facture->enabled) && $object->statut > 0  && ! $object->billed)
					{
						...
						if ($user->rights->salesorder->creer && $object->statut > 2 && empty($conf->global->WORKFLOW_DISABLE_CLASSIFY_BILLED_FROM_ORDER) && empty($conf->global->WORsKFLOW_BILL_ON_SHIPMENT))
						{
					...
					// Set to shipped
					if (($object->statut == 1 || $object->statut == 2) && $user->rights->salesorder->cloturer)
					{
			...
			if ($action != 'presend')
			{
				...
				*/
				...
				$file = $conf->salesorder->dir_output . '/' . $comref . '/' . $comref . '.pdf';
				$relativepath = $comref.'/'.$comref.'.pdf';
				$filedir = $conf->salesorder->dir_output . '/' . $comref;
				$urlsource=$_SERVER["PHP_SELF"]."?id=".$object->id;
				$genallowed=$user->rights->salesorder->creer;
				$delallowed=$user->rights->salesorder->supprimer;

				$somethingshown=$formfile->show_documents('salesorder',$comref,$filedir,$urlsource,$genallowed,$delallowed,$object->modelpdf,1,0,0,28,0,'','','',$soc->default_lang,$hookmanager);
				
				
				//$comref = dol_sanitizeFileName($object->ref);*/
				
				
				/* descomentar para generar pdf de pedido, faltar sacar dinamicamente el ref del pedido y actualizar link de borrado*/
				/*
				$comref = 'CO1401-0005';
			...
			*/
			if ($action == 'presend')
			{
				...
				$fileparams = dol_most_recent_file($conf->salesorder->dir_output . '/' . $ref, preg_quote($ref,'/'));
				...
				if (! $file || ! is_readable($file))
				{
					...
					$result=salesorder_pdf_create($db, $object, GETPOST('model')?GETPOST('model'):$object->modelpdf, $outputlangs, $hidedetails, $hidedesc, $hideref, $hookmanager);
					if ($result <= 0)
					{
						dol_print_error($db,$result);
						exit;
