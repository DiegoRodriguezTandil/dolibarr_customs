...
$sql.= ", f.facnumber, f.fk_currency, f.increment, f.total as total_ht, f.tva as total_tva, f.total_ttc";
...
if ($resql)
{
	...
	//FEDE
	print_liste_field_titre($langs->trans("Divisa"),$_SERVER["PHP_SELF"],"f.fk_currency","",$param,'align="center"',$sortfield,$sortorder);
	//FIN FEDE
	...
	if ($num > 0)
	{
		...
		while ($i < $num)
		{
			...
			print '<td nowrap><a href="'.DOL_URL_ROOT.'/comm/fiche.php?socid='.$objp->socid.'">'.img_object($langs->trans("ShowCompany"),"company").' '.dol_trunc($objp->nom,28).'</a></td>';

			print '<td>'.$objp->fk_currency.'</td>';
