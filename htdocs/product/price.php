...
if ($action == 'update_price' && ! $_POST["cancel"] && ($user->rights->produit->creer || $user->rights->service->creer))
{
	...
	if ($object->updatePrice($object->id, $_POST["fk_currency"],$newprice, $newpricebase, $user, $newvat, $newprice_min, $level, $newnpr, $newpsq) > 0)
	{
		$action = '';
...
// Label
print '<tr><td>'.$langs->trans("Serial Number").'</td><td>'.$object->libelle.'</td>';
