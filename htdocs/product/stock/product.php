...
if ($id > 0 || $ref)
{
	...
	if ($result > 0)
	{
		...
		// Ref
		print '<tr>';
		print '<td width="30%">'.$langs->trans("Part Number").'</td><td>';
		print $form->showrefnav($product,'ref','',1,'ref');
		print '</td>';
		print '</tr>';

		// Label
		print '<tr><td>'.$langs->trans("Serial Number").'</td><td>'.$product->libelle.'</td>';
		print '</tr>';
...
$sql = "SELECT e.rowid, e.label,(select fk_currency from llx_product where rowid=ps.fk_product) as currency, ps.reel, ps.pmp";
...
if ($resql)
{
	...
	while ($i < $num)
	{
		...
		//FEDE
		print '<td align="right">'.$obj->currency.'</td>';
		//FIN FEDE
		// PMP
...
//FEDE
print '<td class="liste_total" align="right">'.$currency.'</td>';
//FIN FEDE
