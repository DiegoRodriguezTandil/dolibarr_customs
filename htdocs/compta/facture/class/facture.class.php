...
class Facture extends CommonInvoice
{
    ...
    function create($user,$notrigger=0,$forceduedate=0)
    {
        ...
        $sql.= ", fk_facture_source, fk_user_author, fk_projet, fk_currency";
        $sql.= ", fk_cond_reglement, fk_mode_reglement, date_lim_reglement, model_pdf, gross_income";
        $sql.= ")";
        $sql.= " VALUES (";
        $sql.= "'(PROV)'";
        $sql.= ", ".$conf->entity;
		...
		//ingresos brutos
        $sql.= ", '".$this->db->idate($datelim)."', '".$this->modelpdf."','".$this->db->escape($this->ingresosbrutos).";')";
        //
    ...
    function fetch($rowid, $ref='', $ref_ext='', $ref_int='')
    {
        ...
        $sql = 'SELECT f.rowid,f.office, f.facnumber,f.ref_client,f.ref_ext,f.ref_int,f.type,f.fk_soc,f.amount,f.tva, f.localtax1, f.localtax2, f.total,f.total_ttc,f.remise_percent,f.remise_absolue,f.remise';
        $sql.= ', f.datef as df';
        ...
        $sql.= ', f.fk_mode_reglement, f.fk_cond_reglement, f.fk_projet, f.fk_currency, f.extraparams, f.gross_income';
        ...
        if ($result)
        {
            if ($this->db->num_rows($result))
            {
				...
				$this->ingresosbrutos       = $obj->gross_income;
    ...
    function fetch_lines()
    {
        ...
        $sql = 'SELECT l.rowid, l.fk_product, l.fk_currency, l.origin_price, l.fk_parent_line, l.label as custom_label, l.description, l.product_type, l.price, l.qty, l.tva_tx, ';
...
class FactureLigne
{
    ...
    {
        $sql = 'SELECT fd.rowid, fd.fk_facture, fd.fk_parent_line, fd.fk_product, fd.fk_currency, fd.origin_price, fd.product_type, fd.label as custom_label, fd.description, fd.price, fd.qty, fd.tva_tx,';
        ...
        if ($result)
        {
			...
			//FEDE
			$this->fk_currency			= $objp->fk_currency;
			$this->origin_price			= $objp->origin_price;
			//FIN FEDE
    ...
    function insert($notrigger=0)
    {
        ...
        $sql.= ' fk_product, fk_currency, product_type, remise_percent, origin_price, subprice, fk_remise_except,';
		...
		//FEDE
		$sql.= "(select price from llx_product_price where rowid=(select max(rowid) from llx_product_price where fk_product=".$this->fk_product." and price_level=(SELECT ifnull(a.price_level,1) FROM llx_societe a join llx_facture b on a.rowid=b.fk_soc where b.rowid=".$this->fk_facture."))),";	
		//FIN FEDE
		
		if($this->origin=='salesorder')
			//$sql.= "(select (SELECT a.price*rate FROM llx_currency_conversion where source=b.fk_currency and target=f.fk_currency order by date desc limit 1) from llx_product_price a join llx_product b on a.fk_product=b.rowid join llx_facture f on f.rowid=".$this->fk_facture." where a.rowid=(select max(rowid) from llx_product_price where fk_product=".$this->fk_product." and price_level=(SELECT ifnull(a.price_level,1) FROM llx_societe a join llx_facture b on a.rowid=b.fk_soc where b.rowid=".$this->fk_facture."))),";
			$sql.= "(select (select sd.subprice*rate from llx_currency_conversion where source=sh.fk_currency and target=fh.fk_currency order by date desc limit 1) from llx_salesorderdet sd join llx_salesorder sh on sd.fk_salesorder=sh.rowid join llx_facture fh on fh.rowid=".$this->fk_facture." where sd.rowid=".$this->origin_id."),";
		else
		...
		if($this->origin=='salesorder')
        	$sql.= "(select (select round(sd.subprice*rate*".$this->qty.",2) from llx_currency_conversion where source=sh.fk_currency and target=fh.fk_currency order by date desc limit 1) from llx_salesorderdet sd join llx_salesorder sh on sd.fk_salesorder=sh.rowid join llx_facture fh on fh.rowid=".$this->fk_facture." where sd.rowid=".$this->origin_id."),";
		else
