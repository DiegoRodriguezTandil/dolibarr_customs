		...
		if (! empty($idprod))
		{
			...
			else
			{
				...
				if (! empty($conf->global->PRODUIT_MULTIPRICES) && $object->client->price_level)
				{
					...
					//FEDE
					$rate=0;
					$resql=$db->query("SELECT rate FROM llx_currency_conversion a join llx_product b on a.source=b.fk_currency join llx_propal c on a.target=c.fk_currency  where  b.rowid=".$idprod." and c.rowid=".$object->id." order by date desc");
...
if ($object->statut == 0 && $user->rights->propal->creer)
{
	if ($action != 'editline')
	{
		...
		//UBICACION PREVIA NUEVO PRODUCTO
...
if ($action == 'statut')
{
	...
	//ACA ESTABA CREAR ORDEN DE VENTA DESDE EL PRESUPUESTO JSCRIPT
	
	
	//SEGUIR PROBANDO ACCIONES TODAVIA NINGUNA LLEGA AL LUGAR CORRECTO
...
if ($action != 'presend')
{
	...
	/*	print '<head>
		...
		<body>
			...
			<input type="button" value=" Nuevo Producto " class="butAction" onclick="open_win()">
	...
	//FIN AGREGAR NUEVO PRODUCTO
		...
		} //ESTE ES UN END DE OTRA ACCION, NO BORRAR
...
if ($action != 'presend')
		...
		<?php $urlid= GETPOST('id'); ?>
        <input type="hidden" value="<?php echo $pricelevel ?>" name="pricelevel" id="pricelevel"/>
		
		<input type="hidden" value="<?php echo $pricelevel ?>" name="pricex" id="pricex"/>
		
		<input type="Submit" value="Crear Nuevo Producto" class="butAction" onclick="open_win()">
		</form>
		</div>
	...
	//FIN AGREGAR NUEVO PRODUCTO Y PASAR NIVEL DE PRECIO A FICHE2.PHP */
