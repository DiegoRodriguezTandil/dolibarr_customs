...
else if ($action ==	'updateshipto' && $user->rights->fournisseur->commande->creer)
{
    $object->setShipto($shiptoid);
	$action="";
}
...
if (! empty($object->id))
{
	...
	*/
	...
	print $langs->trans('Destino del pedido');
	print '</td>';
	if ($action != 'setshipto') print '<td align="right"><a href="'.$_SERVER['PHP_SELF'].'?action=setshipto&amp;id='.$object->id.'">'.img_edit($langs->trans('SetShipto')).'</a></td>';
	print '</tr></table>';
	print '</td><td colspan="2">';
	
	if ($action == 'setshipto')
	{
		$form->form_shipto($_SERVER['PHP_SELF'].'?id='.$object->id, $object->fk_shipto, 'shiptoid');
	}
	else
	{
		print ''.$object->entrepot->getNomUrl().'';
	}
	print '</td>';
	print '</tr>';
	//FIN FEDE
	...
	if (! empty($conf->projet->enabled))
	{
		...
		if ($action == 'classify')
		{
		     //CAMBIADO PARA PODER ASOCIAR A CUALQUIER PROYECTO EN PEDIDO A PROVEEDOR
			...
			$form->form_project($_SERVER['PHP_SELF'].'?id='.$object->id, ($conf->global->PROJECT_CAN_ALWAYS_LINK_TO_ALL_SUPPLIERS)?$object->socid:'-1', $object->fk_project, 'projectid');
		}
		else
		{
