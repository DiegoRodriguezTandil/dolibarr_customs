...
class Project extends CommonObject
{
    ...
    function get_element_list($type)
    {
		...
		if ($type == 'salesorder')
            $sql = "SELECT rowid FROM " . MAIN_DB_PREFIX . "salesorder WHERE fk_statut>0 and fk_projet=" . $this->id;
        if ($type == 'invoice')
     ...
     */
    function getProjectsAuthorizedForUser($user, $mode=0, $list=0, $socid=0, $limit=0)
    {
        $projects = array();
        $temp = array();
