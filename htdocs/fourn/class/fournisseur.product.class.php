...
class ProductFournisseur extends Product
{
	...
	//FEDE
	var $fourn_price_list;             // price for quantity
     ...
     *    @return	int								<0 if KO, >=0 if OK
     */
    function update_buyprice($qty, $buyprice, $pricelist, $user, $price_base_type, $fourn, $availability, $ref_fourn, $tva_tx, $charges=0, $remise_percent=0, $remise=0)
    {
        global $conf,$mysoc;
		...
		//FEDE
		if (empty($pricelist)) $pricelist=0;
		//FIN FEDE
		...
		//FEDE
		$pricelist=price2num($pricelist,'MU');
		// FIN FEDE
        ...
        if ($this->product_fourn_price_id)
        {
			...
			//FEDE
			$sql.= " price_list = ".price2num($pricelist).",";
			// FIN FEDE
			$sql.= " quantity = ".$qty.",";
				...
				if ($resql)
		  		{
		            ...
		            $sql.= "datec, fk_product, fk_soc, ref_fourn, fk_user, price, price_list, quantity, remise_percent, remise, unitprice, tva_tx, charges, unitcharges, fk_availability, entity)";
    ...
    {
        $sql = "SELECT pfp.rowid, pfp.price, pfp.price_list, pfp.quantity, pfp.unitprice, pfp.remise_percent, pfp.remise, pfp.tva_tx, pfp.fk_availability,";
        ...
        if ($resql)
        {
            ...
            if ($obj)
            {
				...
				//FEDE
				$this->fourn_price_list			= $obj->price_list;
    ...
    function list_product_fournisseur_price($prodid, $sortfield='', $sortorder='')
    {
        ...
        $sql.= " pfp.price, pfp.price_list, pfp.quantity, pfp.unitprice, pfp.remise_percent, pfp.remise, pfp.tva_tx, pfp.fk_availability, pfp.charges, pfp.unitcharges";
        $sql.= " FROM ".MAIN_DB_PREFIX."product_fournisseur_price as pfp";
        ...
        if ($resql)
        {
            ...
            while ($record = $this->db->fetch_array($resql))
            {
				...
				//FEDE
				$prodfourn->fourn_price_list		= $record["price_list"];
				//FIN FEDE
    ...
    function find_min_price_product_fournisseur($prodid)
    {
        ...
        $sql.= " pfp.price, pfp.price_list, pfp.quantity, pfp.unitprice, pfp.tva_tx, pfp.charges, pfp.unitcharges";
        ...
        if ($resql)
        {
			...
			//FEDE
			$this->fourn_price_list				= $record["price_list"];
			//FIN FEDE
