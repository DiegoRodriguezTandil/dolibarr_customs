...
require_once DOL_DOCUMENT_ROOT.'/salesorder/class/salesorderstats.class.php';
...
$office=GETPOST('office','int'); if ($office < 0) $office=0;
// Security check
...
if ($mode == 'customer')
{
    $title=$langs->trans("Estadísticas de Ordenes de Venta");
    $dir=$conf->salesorder->dir_temp;
}
if ($mode == 'supplier')
{
...
$stats = new SalesorderStats($db, $socid, $mode,$office, $userid);
...
if (! $mesg)
{
    ...
    $px1->SetYLabel($langs->trans("Número de órdenes"));
    $px1->SetShading(3);
    $px1->SetHorizTickIncrement(1);
    $px1->SetPrecisionY(0);
    $px1->mode='depth';
    $px1->SetTitle($langs->trans("Número de órdenes por mes"));
...
if (! $mesg)
{
    ...
    $px2->SetYLabel($langs->trans("Importe total en órdenes de venta"));
    $px2->SetShading(3);
    $px2->SetHorizTickIncrement(1);
    $px2->SetPrecisionY(0);
    $px2->mode='depth';
    $px2->SetTitle($langs->trans("Importe total de órdenes por mes"));
...
//{
	...
	print '<tr><td align="left">Oficina de Ventas</td><td align="left">';
	if ($mode == 'customer') $filter='s.client in (1,2,3)';
	if ($mode == 'supplier') $filter='s.fournisseur = 1';
	print $form->select_company($office,'office','(s.fk_typent=101)',1);
	//print $form->select_company($socid,'socid',$filter,1);
	print '</td></tr>';
...
print '<td align="center">'.$langs->trans("Número de órdenes").'</td>';
