...
class Propal extends CommonObject
{
    ...
    function __construct($db, $socid="", $propalid=0)
    {
        ...
        $this->labelstatut[2]=(! empty($conf->global->PROPAL_STATUS_SIGNED_LABEL) ? $conf->global->PROPAL_STATUS_SIGNED_LABEL : $langs->trans("Approved"));
        $this->labelstatut[3]=(! empty($conf->global->PROPAL_STATUS_NOTSIGNED_LABEL) ? $conf->global->PROPAL_STATUS_NOTSIGNED_LABEL : $langs->trans("Not Approved"));
    ...
    function fetch($rowid,$ref='')
    {
        ...
        $sql.= ", p.purpose,p.tender_number, p.fk_projet, p.office, p.fk_currency, p.fk_statut";
		//End - gA - 26-07-2013
        ...
        if ($resql)
        {
            if ($this->db->num_rows($resql))
            {
				...
				$this->tender_number		= $obj->tender_number;
                ...
                $sql.= " d.info_bits, d.total_ht, d.total_tva, d.total_localtax1, d.total_localtax2, d.total_ttc, d.fk_product_fournisseur_price as fk_fournprice, d.buy_price_ht as pa_ht, d.special_code, d.rang, d.product_type, d.line_ref, ";
	...
	function setPurpose($purpose)
    {
		...
		dol_syslog("Propal::setPurpose sql=$sql",LOG_DEBUG);
		$resql=$this->db->query($sql);
		if ($resql)
		{
		...
		{
			$this->error=$this->db->error();
			dol_syslog("Propal::setPurpose ".$this->error,LOG_ERR);
			return -1;
		}
    }
    ...
    {
		$sql = "UPDATE ".MAIN_DB_PREFIX."propal";
		$sql.= " SET tender_number = '".$number."'";
		$sql.= " WHERE rowid = ".$this->id;

		dol_syslog("propal::setTenderNumber sql=$sql",LOG_DEBUG);
		$resql=$this->db->query($sql);
		if ($resql)
		{
			$this->tender_number = $number;
			return 1;
		}
		else
		{
			$this->error=$this->db->error();
			dol_syslog("Propal::setTenderNumber ".$this->error,LOG_ERR);
			return -1;
		}
    }
	//FIN FEDE
    ...
    function LibStatut($statut,$mode=1)
    {
        ...
        if ($mode == 4)
        {
			...
			if ($statut==2) return img_picto($langs->trans('Approved'),'statut3').' '.$this->labelstatut[$statut];
            //if ($statut==3) return img_picto($langs->trans('PropalStatusNotSigned'),'statut5').' '.$this->labelstatut[$statut];
			if ($statut==3) return img_picto($langs->trans('Not Approved'),'statut5').' '.$this->labelstatut[$statut];
    ...
    function getLinesArray()
    {
        ...
        if ($resql)
        {
            ...
            while ($i < $num)
            {
				...
				//FEDE
				$this->lines[$i]->fk_currency		= $obj->fk_currency;
				$this->lines[$i]->origin_price		= $obj->origin_price;
				//FIN FEDE
...
class PropaleLigne
{
	...
	function fetch($rowid)
	{
		...
		//FEDE 15-10-2013
		$sql.= ' INNER JOIN '.MAIN_DB_PREFIX.'propal as c ON pd.fk_propal=c.rowid';
		//FEDE
    ...
    function insert($notrigger=0)
    {
        ...
        $sql.= ' (fk_propal, fk_parent_line, label, description, fk_product,fk_currency , product_type,'; 
		$sql.= ' fk_remise_except, qty, tva_tx, localtax1_tx, localtax2_tx, origin_price,';
		...
		//FEDE
		$sql.= "(select price from llx_product_price where rowid=(select max(rowid) from llx_product_price where fk_product=".$this->fk_product." and price_level=(SELECT ifnull(a.price_level,1) FROM llx_societe a join llx_propal b on a.rowid=b.fk_soc where b.rowid=".$this->fk_propal."))),";	
		//FIN FEDE
