...
$sql.= ' f.rowid as facid, f.facnumber, f.fk_currency, f.type, f.increment, f.total as total_ht, f.tva as total_tva, f.total_ttc,';
$sql.= "(SELECT f.total*rate FROM llx_currency_conversion WHERE source=f.fk_currency AND target='".$conf->currency."' and date=(SELECT MAX(date) FROM llx_currency_conversion WHERE source=f.fk_currency AND target='".$conf->currency."')) as T_total_ht,";
$sql.= "(SELECT f.tva*rate FROM llx_currency_conversion WHERE source=f.fk_currency AND target='".$conf->currency."' and date=(SELECT MAX(date) FROM llx_currency_conversion WHERE source=f.fk_currency AND target='".$conf->currency."')) as T_total_tva,";
$sql.= "(SELECT f.total_ttc*rate FROM llx_currency_conversion WHERE source=f.fk_currency AND target='".$conf->currency."' and date=(SELECT MAX(date) FROM llx_currency_conversion WHERE source=f.fk_currency AND target='".$conf->currency."')) as T_total_ttc,";
...
if ($resql)
{
    ...
    print_liste_field_titre($langs->trans('Company'),$_SERVER['PHP_SELF'],'s.nom','',$param,'',$sortfield,$sortorder);
	 print_liste_field_titre($langs->trans('Currency'),$_SERVER['PHP_SELF'],'f.fk_currency','',$param,'',$sortfield,$sortorder);
    ...
    if ($num > 0)
    {
        ...
        while ($i < min($num,$limit))
        {
			...
			print '<td align="right">'.$objp->fk_currency.'</td>';
            ...
            print '<td align="right">'.(! empty($paiement)?price($paiement):'&nbsp;').'</td>';
            ...
            //print "<td>&nbsp;</td>";
            print "</tr>\n";
            $total_ht+=$objp->T_total_ht;
            $total_tva+=$objp->T_total_tva;
            $total_ttc+=$objp->T_total_ttc;
            $totalrecu+=$paiement;
            $i++;
        }
        ...
        if (($offset + $num) <= $limit)
        {
            ...
            print '<td class="liste_total" colspan="4" align="left">'.$langs->trans('Total').' en '.$langs->trans('Currency'.$conf->currency).'</td>';
			print '<td class="liste_total" align="center">&nbsp;</td>';
            print '<td class="liste_total" align="right">'.price($total_ht).'</td>';
            print '<td class="liste_total" align="right">'.price($total_tva).'</td>';
