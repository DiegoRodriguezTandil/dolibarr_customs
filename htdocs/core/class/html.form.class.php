...
class Form
{
    ...
    function select_produits_do($selected='',$htmlname='productid',$filtertype='',$limit=20,$price_level=0,$filterkey='',$status=1,$finished=2,$disableout=0)
    {
        ...
        $sql = "SELECT ";
        $sql.= " p.rowid, p.label, p.ref, p.description, p.fk_product_type, p.price, p.fk_currency, p.price_ttc, p.price_base_type, p.tva_tx, p.duration, p.stock";
        ...
        if ($result)
        {
            ...
            while ($num && $i < $num)
            {
				...
				if(!empty($objp->price_by_qty) && $objp->price_by_qty == 1) { // Price by quantity will return many prices for the same product
					$sql = "SELECT rowid, quantity, price, unitprice,fk_currency, remise_percent, remise";
	...
	function _construct_product_list_option(&$objp, &$opt, &$optJson, $price_level, $selected) {
		...
		//FEDE 10-10-2013
		$currencytext=$langs->trans($objp->fk_currency);
        $currencytextnoent=$langs->transnoentities($objp->fk_currency);
        
		if (dol_strlen($currencytext) > 10) $currencytext=$objp->fk_currency;	// If text is too long, we use the short code
        if (dol_strlen($currencytextnoent) > 10) $currencytextnoent=$objp->fk_currency;   // If text is too long, we use the short code
		//FIN FEDE 10-10-2013
    ...
    function select_produits_fournisseurs_do($socid,$selected='',$htmlname='productid',$filtertype='',$filtre='',$filterkey='',$statut=-1,$disableout=0)
    {
        ...
        $sql = "SELECT p.rowid, p.label, p.ref, p.price, p.fk_currency, p.duration,";
        ...
        if ($result)
        {
            ...
            while ($i < $num)
            {
                ...
                {
                    $currencytext=$langs->trans("Currency".$objp->fk_currency);
                    $currencytextnoent=$langs->transnoentities("Currency".$objp->fk_currency);
                    if (dol_strlen($currencytext) > 10) $currencytext=$objp->fk_currency;   // If text is too long, we use the short code
                    if (dol_strlen($currencytextnoent) > 10) $currencytextnoent=$objp->fk_currency;   // If text is too long, we use the short code
	...
	//FEDE
	function form_currency($page,  $selected='', $htmlname='currencyid')
    {
        global $langs;
        ...
        {
            print '<form method="post" action="'.$page.'">';
            print '<input type="hidden" name="action" value="updatecurrency">';
            print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
            print '<table class="nobordernopadding" cellpadding="0" cellspacing="0">';
            print '<tr><td>';
			print $this->selectcurrency($selected,$htmlname);
            //print select_currency($selected,$htmlname);
            print '</td>';
            print '<td align="left"><input type="submit" class="button" value="'.$langs->trans("Modify").'"></td>';
            print '</tr></table></form>';
        }
    }
	function form_shipto($page,  $selected='', $htmlname='shiptoid')
    {
        global $langs;
        ...
        {
            print '<form method="post" action="'.$page.'">';
            print '<input type="hidden" name="action" value="updateshipto">';
            print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
            print '<table class="nobordernopadding" cellpadding="0" cellspacing="0">';
            print '<tr><td>';
			print $this->select_warehouse($selected,$htmlname);
            //print select_currency($selected,$htmlname);
            print '</td>';
            print '<td align="left"><input type="submit" class="button" value="'.$langs->trans("Modify").'"></td>';
            print '</tr></table></form>';
    ...
    function select_warehouse($selected='',$htmlname='shiptoid')
    {
        print $this->selectwarehouse($selected,$htmlname);
    }
    /**
	...
	function load_cache_warehouses()
    {
    	global $langs;

    	$langs->load("dict");

    	if (count($this->cache_warehouses)) return 0;    // Cache deja charge
    ...
    function get_val_currency($selected='',$htmlname='currency_id')
    {
        ...
        $this->load_set_currency($selected);
        
        $out='';
        
        if ($selected=='euro' || $selected=='euros') $selected='EUR';   // Pour compatibilite
        
        $out="<span>".$this->cache_currencies[$selected]["label"];
        $out.= ' ('.getCurrencySymbol($selected).')';
        $out.="</span>";
     ...
     *      @return     int             Nb of loaded lines, 0 if already loaded, <0 if KO
     */
    function load_set_currency($currenci)
    {
        global $langs;
    ...
    function selectcurrency($selected='',$htmlname='currency_id')
    {
	 ...
	 function selectwarehouse($selected='',$htmlname='shiptoid')
    {
        global $conf,$langs,$user;

        $langs->load("dict");

        $this->load_cache_warehouses();

        $out='';

        $out.= '<select class="flat" name="'.$htmlname.'">';
        foreach ($this->cache_warehouses as $index => $warehouse)
        {
        	if ($selected && $selected == $index)
        	{
        		$out.= '<option value='.$index.' selected="selected">';
        	}
        	else
        	{
