...
class Product extends CommonObject
{
	...
	function __construct($db)
	{
		...
		//BEGIN FEDE 01/08/2013
		$this->fk_currency='ARS';
		$this->measure=0;
		//END FEDE 01/08/2013
		
		//ALMACEN HARCODEADO, BORRAR Y AGREGAR COMBO
		$this->fk_entrepot='1';
		//END ALMACEN HARCODEADO, BORRAR Y AGREGAR COMBO
	}
	...
	function update($id, $user, $notrigger=false, $action='update')
	{
		...
		//BEGIN FEDE 01/08/2013
		$sql.= ",fk_currency = '" . $this->fk_currency."'";
		$sql.= ",fk_entrepot = " . $this->fk_entrepot;
		$sql.= ",measure_unit = " . $this->measure;
		//END FEDE 01/08/2013
	 ...
	 * 	@return		int						<0 if KO, >0 if OK
	 */
	function updatePrice($id, $currency, $newprice, $newpricebase, $user, $newvat='',$newminprice='', $level=0, $newnpr=0, $newpsq=0)
	{
		global $conf,$langs;
	...
	function fetch($id='',$ref='',$ref_ext='')
	{
		...
		$sql = "SELECT rowid, ref, label, description, note, customcode, fk_country,fk_currency,fk_entrepot, price, price_ttc,";
		//END FEDE 01/08/2013
		...
		$sql.= " weight, weight_units, length, length_units, surface, surface_units, volume, volume_units, measure_unit, barcode, fk_barcode_type, finished,";
		...
		if ( $resql )
		{
			if ($this->db->num_rows($resql) > 0)
			{
				...
				if($this->fk_entrepot>0)
				{
					...
					$this->entrepot->fetch($this->fk_entrepot);
				}
	...
	function getLibFinished()
	{
		...
		if ($this->finished == '2') return $langs->trans("Servicio");
		return '';
	}
