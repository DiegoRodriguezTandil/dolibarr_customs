		...
		if (! empty($idprod))
		{
			...
			else
			{
				...
				if (! empty($conf->global->PRODUIT_MULTIPRICES) && ! empty($object->client->price_level))
				{
					//FEDE
					$rate=0;
					$resql=$db->query("SELECT rate FROM llx_currency_conversion a join llx_product b on a.source=b.fk_currency join llx_commande c on a.target=c.fk_currency  where  b.rowid=".$idprod." and c.rowid=".$object->id." order by date desc");
	...
	if ($action == 'create' && $user->rights->commande->creer)
	{
		...
		if (! empty($origin) && ! empty($originid))
		{
			...
			else
			{
				...
				//FEDE
				$currency          	= (!empty($objectsrc->fk_currency)?$objectsrc->fk_currency:'');
				//FIN FEDE
		...
		*/
		...
		//FEDE 16-10-2013
		$var=!$var;
		if (empty($currency))$currency=$conf->currency;
		print '<tr '.$bc[$var].'><td>'.$langs->trans("Currency").'</td><td>';
		$form->select_currency($currency,'fk_currency');
		print '</td></tr>'."\n";
		//FIN FEDE
	...
	else
	{
		...
		if ($object->id > 0)
		{
			...
			*/
			...
			print 'Objeto';
			print '</td>';
			if ($action != 'editpurpose') print '<td align="right"><a href="'.$_SERVER["PHP_SELF"].'?action=editpurpose&amp;id='.$object->id.'">'.img_edit($langs->trans('SetPurpose'),1).'</a></td>';
			print '</tr></table>';
			print '</td><td colspan="2">';
			if ($action == 'editpurpose')
			{
				print '<form name="setpurpose" action="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'" method="post">';
				print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
				print '<input type="hidden" name="action" value="setpurpose">';
				//print var_dump($shipment_location);
				print ''.$form->editfieldval('purpose',$purpose,$object->purpose, $object,true,'string:60', '', null, null,'');
				//print '<input type="submit" class="button" value="'.$langs->trans('Modify').'">';
				print '</form>';
			}
			else
			{
				print $object->purpose;
			}
			print '</td></tr>';
			//FIN FEDE
			...
			print '</td>';
			if ($action != 'edittender_number') print '<td align="right"><a href="'.$_SERVER["PHP_SELF"].'?action=edittender_number&amp;id='.$object->id.'">'.img_edit($langs->trans('SetTenderNumber'),1).'</a></td>';
			print '</tr></table>';
			print '</td><td colspan="2">';
			if ($action == 'edittender_number')
			{
				print '<form name="settender_number" action="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'" method="post">';
				print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
				print '<input type="hidden" name="action" value="set_tender_number">';
				print ''.$form->editfieldval('tender_number',$tender_number,$object->tender_number, $object,true,'string:30', '', null, null,'');
				print '</form>';
			}
			else
			{
				print $object->tender_number;
			}
			print '</td></tr>';
			//FIN FEDE
			...
			if ($action != 'presend')
			{
				if ($user->societe_id == 0 && $action <> 'editline')
				{
					...
					*/
					...
					//CREAR ORDEN DE VENTA DESDE PEDIDO
					...
					// FIN CREAR ORDEN DE VENTA DESDE PEDIDO
