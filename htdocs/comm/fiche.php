...
//FEDE
if (! empty($conf->commande->enabled)) require_once DOL_DOCUMENT_ROOT.'/salesorder/class/salesorder.class.php';
//FIN FEDE
...
if ($id > 0)
{
	...
	if (! empty($conf->propal->enabled) && $user->rights->propal->lire)
	{
		...
		$sql = "SELECT s.nom, s.rowid, p.rowid as propalid, p.fk_statut, p.total_ht, p.ref, p.remise, p.fk_currency, ";
		...
		if ($resql)
		{
            ...
            if ($num > 0)
            {
                ...
                print '<tr class="liste_titre">';
    			print '<td colspan="5"><table width="100%" class="nobordernopadding"><tr><td>'.$langs->trans("LastPropals",($num<=$MAXLIST?"":$MAXLIST)).'</td><td align="right"><a href="'.DOL_URL_ROOT.'/comm/propal/list.php?socid='.$object->id.'">'.$langs->trans("AllPropals").' ('.$num.')</a></td>';
			...
			while ($i < $num && $i < $MAXLIST)
			{
				...
				print '<td align="right" width="120">'.$objp->fk_currency.'</td>';
	...
	if (! empty($conf->commande->enabled) && $user->rights->commande->lire)
	{
		...
		$sql.= " c.rowid as cid, c.total_ht, c.ref, c.fk_statut, c.facture, c.fk_currency,";
		...
		if ($resql)
		{
			...
			if ($num > 0)
			{
				...
				$sql2 = 'SELECT s.nom, s.rowid as socid, s.client, c.rowid, c.ref, c.total_ht, c.ref_client, c.fk_currency,';
				...
				print '<tr class="liste_titre">';
				print '<td colspan="5"><table width="100%" class="nobordernopadding"><tr><td>'.$langs->trans("LastOrders",($num<=$MAXLIST?"":$MAXLIST)).'</td><td align="right"><a href="'.DOL_URL_ROOT.'/commande/liste.php?socid='.$object->id.'">'.$langs->trans("AllOrders").' ('.$num.')</a></td>';
	...
	{
		$salesorder_static=new Salesorder($db);

		$sql = "SELECT s.nom, s.rowid, c.fk_currency, ";
		$sql.= " c.rowid as cid, c.total_ht, c.ref, c.fk_statut, c.facture,";
		$sql.= " c.date_salesorder as dc";
		...
		if ($resql)
		{
			...
			if ($num > 0)
			{
				// Check if there are orders billable
				$sql2 = 'SELECT s.nom, s.rowid as socid, s.client, c.rowid, c.ref, c.total_ht, c.ref_client, c.fk_currency,';
				$sql2.= ' c.date_valid, c.date_salesorder, c.date_livraison, c.fk_statut, c.facture as facturee';
				$sql2.= ' FROM '.MAIN_DB_PREFIX.'societe as s';
				$sql2.= ', '.MAIN_DB_PREFIX.'salesorder as c';
				$sql2.= ' WHERE c.fk_soc = s.rowid';
				$sql2.= ' AND s.rowid = '.$object->id;
				// Show orders with status validated, shipping started and delivered (well any order we can bill)
				$sql2.= " AND ((c.fk_statut IN (1,2)) OR (c.fk_statut = 3 AND c.facture = 0))";
				...
				print '<tr class="liste_titre">';
				print '<td colspan="5"><table width="100%" class="nobordernopadding"><tr><td>'.$langs->trans("Ultimas órdenes de venta",($num<=$MAXLIST?"":$MAXLIST)).'</td><td align="right"><a href="'.DOL_URL_ROOT.'/commande/liste.php?socid='.$object->id.'">'.$langs->trans("AllOrders").' ('.$num.')</a></td>';
				print '<td width="20px" align="right"><a href="'.DOL_URL_ROOT.'/salesorder/stats/index.php?socid='.$object->id.'">'.img_picto($langs->trans("Statistics"),'stats').'</a></td>';
				//if($num2 > 0) print '<td width="20px" align="right"><a href="'.DOL_URL_ROOT.'/commande/orderstoinvoice.php?socid='.$object->id.'">'.img_picto($langs->trans("CreateInvoiceForThisCustomer"),'object_bill').'</a></td>';
				//else print '<td width="20px" align="right"><a href="#">'.img_picto($langs->trans("NoOrdersToInvoice"),'object_bill').'</a></td>';
				print '</tr></table></td>';
				print '</tr>';
			...
			while ($i < $num && $i < $MAXLIST)
			{
				...
				$var=!$var;
				print "<tr $bc[$var]>";
				print '<td nowrap="nowrap"><a href="'.DOL_URL_ROOT.'/salesorder/fiche.php?id='.$objp->cid.'">'.img_object($langs->trans("ShowOrder"),"order").' '.$objp->ref."</a>\n";
				print '</td><td align="right" width="80">'.dol_print_date($db->jdate($objp->dc),'day')."</td>\n";
				print '<td align="right" width="120">'.$objp->fk_currency.'</td>';
				print '<td align="right" width="120">'.price($objp->total_ht).'</td>';
				print '<td align="right" width="100">'.$salesorder_static->LibStatut($objp->fk_statut,$objp->facture,5).'</td></tr>';
				$i++;
			}
			$db->free($resql);
	...
	//FEDE Agrega el botón para crear la órden de venta
	if (! empty($conf->salesorder->enabled) && $user->rights->salesorder->creer)
	{
		$langs->load("salesorders");
		print '<a class="butAction" href="'.DOL_URL_ROOT.'/salesorder/fiche.php?socid='.$object->id.'&amp;action=create">'.$langs->trans("AddSalesOrder").'</a>';
	}
	//FIN FEDE
	...
	if ($user->societe_id == 0)
	{
		...
		if (! empty($conf->facture->enabled))
		{
			if ($user->rights->facture->creer)
			{
				...
				{
					//SACAR BOTONES DE FACTURACION EN LA SOLAPA CLIENTE
