...
if (empty($reshook))
{
	...
	//FEDE
	if ($action == 'updateshipto')
    {
        $object->setWarehouse($entrepotid);
        $action="";
    }
	//FIN FEDE
    ...
    if ($action == 'add' && ($user->rights->produit->creer || $user->rights->service->creer))
    {
        ...
        if (! $error)
        {
			...
			$object->measure            	= GETPOST('measure_unit');
			//FIN FEDE
...
else
{
    ...
    if ($action == 'create' && ($user->rights->produit->creer || $user->rights->service->creer))
    {
		...
		//Si es un nuevo servicio mostrar leyenda Part Number / Serial Number, si es un nuevo producto mostrar leyenda Part Number
         if ($action == 'create' && ($user->rights->produit->creer || $user->rights->service->creer))
	     
		 if ($type==1)  {
						print '<tr><td class="fieldrequired">'.$langs->trans("Part Number/ Serial Number").'</td><td><input name="ref" size="40" value="'.GETPOST('libelle').'"></td></tr>';
						}
						else
						{
						
							//if ($type==2)  {
											print '<tr><td class="fieldrequired">'.$langs->trans("Part Number").'</td><td><input name="ref" size="40" value="'.GETPOST('libelle').'"></td></tr>';
							//			    }
						}
        // fin cambio leyenda nuevo servicio/producto		
			
			
			
       // print '<td class="fieldrequired" width="20%">'.$langs->trans("Part Number").'</td><td><input name="ref" size="40" maxlength="32" value="'.$tmpcode.'">';
        if ($_error)
        {
        ...
        // Label
       // print '<tr><td class="fieldrequired">'.$langs->trans("Serial Number").'</td><td><input name="libelle" size="40" value="'.GETPOST('libelle').'"></td></tr>';
	   //CAMBIAR LEYENDA DE SERIAL NUMBER A CODIGO SOLO PARA SERVICIOS
	   
	   if ($action == 'create' && ($user->rights->produit->creer || $user->rights->service->creer))
	     
		 if ($type==1)  {
						print '<tr><td class="fieldrequired">'.$langs->trans("Código").'</td><td><input name="libelle" size="40" value="'.GETPOST('libelle').'"></td></tr>';
						}
		else
						{
        ...
        if ($type != 1)
        {
            ...
            $statutarray=array('1' => $langs->trans("Finished"), '0' => $langs->trans("RowMaterial"),'2' => $langs->trans("Servicio"));
        ...
        if ($type != 1)	// Le poids et le volume ne concerne que les produits et pas les services
        {
			...
			//FEDE Measure Unit
            print '<tr><td>'.$langs->trans("Unidad de Medida").'</td><td>';
            print $formproduct->select_measuring_units("measure_unit","measure");
            print '</td></tr>';
        }
		else
		{
			//FEDE Measure Unit
            print '<tr><td></td><td>';
            print '<input type="hidden" name="measure_unit" size="4" value="0">';
            print '</td></tr>';
		}
		...
		//BEGIN FEDE 
		print '<tr><td>'.$langs->trans("Divisa").'</td><td>';
    ...
    else if ($id || $ref)
    {
        ...
        if ($action == 'edit' && ($user->rights->produit->creer || $user->rights->service->creer))
        {
            ...
            // Ref
            //print '<tr><td width="15%" class="fieldrequired">'.$langs->trans("Part Number").'</td><td colspan="2"><input name="ref" size="40" maxlength="32" value="'.$object->ref.'"></td></tr>';
		   ...
		   //Si es un nuevo servicio mostrar leyenda Part Number / Serial Number, si es un nuevo producto mostrar leyenda Part Number
			...
			if ($object->isservice()) {
							...
							print '<tr><td width="20%" class="fieldrequired">'.$langs->trans("Part Number/ Serial Number").'</td><td colspan="2"><input name="ref" size="40" maxlength="32" value="'.$object->ref.'"></td></tr>';
							}
							else
							{
        ...
        // fin cambio leyenda nuevo servicio/producto
            ...
            // Label
           // print '<tr><td class="fieldrequired">'.$langs->trans("Serial Number").'</td><td colspan="2"><input name="libelle" size="40" value="'.$object->libelle.'"></td></tr>';
		    ...
		    //CAMBIAR LEYENDA DE SERIAL NUMBER A CODIGO SOLO PARA SERVICIOS
				...
				if ($action == 'edit' && ($user->rights->produit->creer || $user->rights->service->creer))
				    ...
				    if ($object->isservice()) {
												print '<tr><td class="fieldrequired">'.$langs->trans("Código").'</td><td colspan="2"><input name="libelle" size="40" value="'.$object->libelle.'"></td></tr>';
											  }
            ...
            if($object->type!=1)
            {
                ...
                $statutarray=array('-1'=>'&nbsp;', '1' => $langs->trans("Finished"), '0' => $langs->trans("RowMaterial"),'2' => $langs->trans("Servicio"));
            ...
            else
            {
                ...
                print '</td></tr>';
				//FEDE Measure unit
                print '<tr><td>'.$langs->trans("Unidad de Medida").'</td><td colspan="2">';
                print $formproduct->select_measuring_units("measure_unit", "measure", $object->measure);
                print '</td></tr>';
            }
        ...
        else
        {
            ...
            // Ref
			//Adecuar la leyenda de acuerdo si es un producto o servicio
			
			if ($object->isservice()) {			
			
            print '<td width="20%">'.$langs->trans("Part Number/ Serial Number").'</td><td colspan="'.(2+(($showphoto||$showbarcode)?1:0)).'">';
			...
			print '<td width="15%">'.$langs->trans("Part Number").'</td><td colspan="'.(2+(($showphoto||$showbarcode)?1:0)).'">';
            print $form->showrefnav($object,'ref','',1,'ref');
            print '</td>';
			...
			{				
            print '<tr><td>'.$langs->trans("Código").'</td><td colspan="2">'.$object->libelle.'</td>';
			}
			
			else
			
			{				
            print '<tr><td>'.$langs->trans("Serial Number").'</td><td colspan="2">'.$object->libelle.'</td>';
			}
            ...
            else
            {
                ...
                if ($object->measure != '')
                {
                    print measuring_units_string($object->measure,"measure");
                }
                else
                {
                    print '&nbsp;';
			...
			//BEGIN FEDE 01/08/2013
			print '<tr><td>'.$langs->trans("Divisa").'</td><td>' .$langs->trans("Currency".$object->fk_currency).'</td></tr>';
			//END FEDE 01/08/2013
			
			// FEDE Almacén
			print '<tr><td height="10">';
			print '<table class="nobordernopadding" width="100%"><tr><td>';
			print $langs->trans('Depósito origen');
			print '</td>';
			if ($action != 'setentrepot') print '<td align="right"><a href="'.$_SERVER['PHP_SELF'].'?action=setentrepot&amp;id='.$object->id.'">'.img_edit($langs->trans('SetEntrepot')).'</a></td>';
			print '</tr></table>';
			print '</td><td colspan="2">';
			
			if ($action == 'setentrepot')
			{
				$form->form_shipto($_SERVER['PHP_SELF'].'?id='.$object->id, $object->fk_entrepot, 'entrepotid');
			}
			else
			{
				if($object->fk_entrepot>0)print ''.$object->entrepot->getNomUrl().'';
			}
			print '</td>';
			print '</tr>';
			//FIN FEDE
