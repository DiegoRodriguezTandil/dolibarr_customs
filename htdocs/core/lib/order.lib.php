...
function commande_prepare_head($object)
{
	...
	//FEDE
		$head[$h][0] = DOL_URL_ROOT.'/commande/poliza.php?id='.$object->id;
		$head[$h][1] = $langs->trans('Poliza');
		$head[$h][2] = 'poliza';
		$h++;
		
		$head[$h][0] = DOL_URL_ROOT.'/commande/gastos.php?id='.$object->id;
		$head[$h][1] = $langs->trans('Gastos');
		$head[$h][2] = 'gastos';
		$h++;
	//FIN FEDE
...
//FEDE
function salesorder_prepare_head($object)
{
	global $langs, $conf, $user;
	if (! empty($conf->expedition->enabled)) $langs->load("sendings");
	...
	{
		$head[$h][0] = DOL_URL_ROOT.'/salesorder/fiche.php?id='.$object->id;
		$head[$h][1] = $langs->trans("SalesOrder");
		$head[$h][2] = 'order';
		$h++;
	}
	...
	|| ($conf->livraison_bon->enabled && $user->rights->expedition->livraison->lire))
	{
		$head[$h][0] = DOL_URL_ROOT.'/expedition/shipment2.php?id='.$object->id;
		if ($conf->expedition_bon->enabled) $text=$langs->trans("Sendings");
		if ($conf->expedition_bon->enabled && $conf->livraison_bon->enabled) $text.='/';
		if ($conf->livraison_bon->enabled)  $text.=$langs->trans("Receivings");
		$head[$h][1] = $text;
	...
	if (! empty($conf->global->MAIN_USE_PREVIEW_TABS))
	{
		$head[$h][0] = DOL_URL_ROOT.'/salesorder/apercu.php?id='.$object->id;
		$head[$h][1] = $langs->trans("Preview");
		$head[$h][2] = 'preview';
		$h++;
	}
	...
	if (empty($conf->global->MAIN_DISABLE_CONTACTS_TAB))
	{
		$head[$h][0] = DOL_URL_ROOT.'/salesorder/contact.php?id='.$object->id;
		$head[$h][1] = $langs->trans('ContactsAddresses');
		$head[$h][2] = 'contact';
		$h++;
	}
	...
	//FEDE
		$head[$h][0] = DOL_URL_ROOT.'/salesorder/poliza.php?id='.$object->id;
		$head[$h][1] = $langs->trans('Poliza');
		$head[$h][2] = 'poliza';
		$h++;
		
		$head[$h][0] = DOL_URL_ROOT.'/salesorder/gastos.php?id='.$object->id;
		$head[$h][1] = $langs->trans('Gastos');
		$head[$h][2] = 'gastos';
		$h++;
	//FIN FEDE
    ...
    $head[$h][0] = DOL_URL_ROOT.'/salesorder/document.php?id='.$object->id;
	/*$filesdir = $conf->commande->dir_output . "/" . dol_sanitizeFileName($commande->ref);
	include_once DOL_DOCUMENT_ROOT.'/core/lib/files.lib.php';
	$listoffiles=dol_dir_list($filesdir,'files',1);
	$head[$h][1] = (count($listoffiles)?$langs->trans('DocumentsNb',count($listoffiles)):$langs->trans('Documents'));*/
	...
	if (empty($conf->global->MAIN_DISABLE_NOTES_TAB))
	{
		$head[$h][0] = DOL_URL_ROOT.'/salesorder/note.php?id='.$object->id;
		$head[$h][1] = $langs->trans('Notes');
		$head[$h][2] = 'note';
		$h++;
	}

	$head[$h][0] = DOL_URL_ROOT.'/salesorder/info.php?id='.$object->id;
	$head[$h][1] = $langs->trans("Info");
	$head[$h][2] = 'info';
	$h++;
