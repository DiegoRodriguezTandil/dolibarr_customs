    ...
    if ($action == 'add' && ($user->rights->produit->creer || $user->rights->service->creer))
    {
        ...
        if (! $error)
        {
            ...
            if ($id > 0)
            {
				...
				//AGREGAR NUEVO PRODUCTO Y NIVEL DE PRECIO DEL TERCERO
				
				//header("Location: ".$_SERVER['PHP_SELF']."?id=".$id);
				
				//nuevo
				$prxx= GETPOST('priceleveltoshow');
				$pricein = GETPOST('price');
				
				header("Location: ../product/pricelevel.php?id=".$id ."&prxx=" . $prxx ."&price=" . $pricein);
              ...
              //FIN //AGREGAR NUEVO PRODUCTO Y NIVEL DE PRECIO DEL TERCERO
...
else
{
    ...
    if ($action == 'create' && ($user->rights->produit->creer || $user->rights->service->creer))
    {
        ...
        print '<form action="fiche2.php" method="post">';
		...
		//NIVEL DE PRECIO 
			...
			print '<tr><td>Nivel de Precio</td><td>';
			$prlevel2 =  $_GET['pricelevel'];
			//echo $prlevel2; ?>
			<input type="hidden" name="priceleveltoshow" size="4" value="<?php echo $prlevel2; ?>"><B>Sólo se creará el precio para la categoría <?php echo $prlevel2; ?></B>
			...
			//BEGIN FEDE 			
			print '<tr><td>Precio</td><td><input name="price" size="4">';
			print $form->select_currency($object->fk_currency,'fk_currency');
			print '</tr>';
			//END FEDE 
			
			//FIN PRECIO Y  NIVEL DE PRECIO
