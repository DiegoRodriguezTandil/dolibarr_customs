...
class InterfaceActionsAuto
{
    ...
    function run_trigger($action,$object,$user,$langs,$conf)
    {
		...
		if ($action == 'POLICY_MODIFY')
        {
            ...
            if (empty($object->actionmsg2)) $object->actionmsg2=$langs->transnoentities("Cancelar póliza",$object->nom);
            $object->actionmsg=$langs->transnoentities("Cancelar póliza",$object->nom);
		...
		elseif ($action == 'SALESORDER_VALIDATE')
        {
            dol_syslog("Trigger '".$this->name."' for action '$action' launched by ".__FILE__.". id=".$object->id);
			...
			$object->actiontypecode='AC_OTH_AUTO';
            if (empty($object->actionmsg2)) $object->actionmsg2=$langs->transnoentities("SalesOrderValidatedInDolibarr",$object->ref);
            $object->actionmsg=$langs->transnoentities("SalesOrderValidatedInDolibarr",$object->ref);
            $object->actionmsg.="\n".$langs->transnoentities("Author").': '.$user->login;
        ...
        */
        ...
        if ($ok)
        {
			...
			if($action == 'POLICY_MODIFY')	$societeforaction->fetch($object->fk_soc);
			...
			if($action == 'POLICY_MODIFY')
			{
				$actioncomm->datep       = $object->expiration_date;
				$actioncomm->datef       = $object->expiration_date;
			}
