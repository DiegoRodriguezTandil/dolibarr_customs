...
if (! empty($conf->facture->enabled) && $user->rights->facture->lire)
{
	$sql  = "SELECT f.facnumber, f.fk_currency, f.rowid, f.total_ttc, f.type,";
	$sql.= " s.nom, s.rowid as socid";
	if (!$user->rights->societe->client->voir && !$socid) $sql.= ", sc.fk_soc, sc.fk_user ";
	//FEDE
	$sql.= ",(SELECT f.total_ttc*rate FROM llx_currency_conversion WHERE source=f.fk_currency AND target='".$conf->currency."' and date=(SELECT MAX(date) FROM llx_currency_conversion WHERE source=f.fk_currency AND target='".$conf->currency."')) as Total ";
	//FIN FEDE
	...
	if ( $resql )
	{
		...
		print '<tr class="liste_titre">';
		print '<td colspan="4">'.$langs->trans("CustomersDraftInvoices").($num?' ('.$num.')':'').'</td></tr>';
		if ($num)
		{
			...
			print '<tr class="liste_total"><td align="left">'.$langs->trans("Total").' en '.$langs->trans('Currency'.$conf->currency).'</td>';
			print '<td colspan="3" align="right">'.price($tot_ttc).'</td>';
			print '</tr>';
		}
		else
		{
			print '<tr colspan="4" '.$bc[$var].'><td>'.$langs->trans("NoInvoice").'</td></tr>';
		}
		print "</table><br>";
		$db->free($resql);
	}
...
if (! empty($conf->fournisseur->enabled) && $user->rights->fournisseur->facture->lire)
{
	...
	$sql.= " s.nom, s.rowid as socid,";
	//FEDE
	$sql.= "(SELECT f.total_ht*rate FROM llx_currency_conversion WHERE source=f.fk_currency AND target='".$conf->currency."' and date=(SELECT MAX(date) FROM llx_currency_conversion WHERE source=f.fk_currency AND target='".$conf->currency."')) as Total ";
	//FIN FEDE
	...
	if ( $resql )
	{
		...
		print '<table class="noborder" width="100%">';
		print '<tr class="liste_titre">';
		print '<td colspan="4">'.$langs->trans("SuppliersDraftInvoices").($num?' ('.$num.')':'').'</td></tr>';
		if ($num)
		{
			$companystatic=new Societe($db);
...
if (! empty($conf->facture->enabled) && $user->rights->facture->lire)
{
	...
	$sql = "SELECT f.rowid, f.fk_currency, f.facnumber, f.fk_statut, f.type, f.total, f.total_ttc, f.paye, f.tms,";
	...
	if ($resql)
	{
		...
		print '<table class="noborder" width="100%">';
		print '<tr class="liste_titre"><td colspan="3">'.$langs->trans("BoxTitleLastCustomerBills",$max).'</td>';
...
if (! empty($conf->fournisseur->enabled) && $user->rights->fournisseur->facture->lire)
{
	...
	$sql = "SELECT ff.rowid, ff.fk_currency, ff.facnumber, ff.fk_statut, ff.libelle, ff.total_ht, ff.total_ttc, ff.tms, ff.paye";
	$sql.= ", s.nom, s.rowid as socid";
	$sql.= ", SUM(pf.amount) as am";
	...
	if ($resql)
	{
		...
		print '<table class="noborder" width="100%">';
		print '<tr class="liste_titre"><td colspan="3">'.$langs->trans("BoxTitleLastSupplierBills",$max).'</td>';
		if (! empty($conf->global->MAIN_SHOW_HT_ON_SUMMARY)) print '<td align="right">'.$langs->trans("AmountHT").'</td>';
		print '<td align="right">'.$langs->trans("AmountTTC").'</td>';
		print '<td align="right">'.$langs->trans("DateModificationShort").'</td>';
		print '<td width="16">&nbsp;</td>';
...
if (! empty($conf->facture->enabled) && $user->rights->facture->lire)
{
	...
	$sql = "SELECT f.rowid, f.fk_currency, f.facnumber, f.fk_statut, f.datef, f.type, f.total, f.total_ttc, f.paye, f.tms,";
	$sql.= " f.date_lim_reglement as datelimite,";
	$sql.= " s.nom, s.rowid as socid,";
	$sql.= " sum(pf.amount) as am";
	$sql.= " FROM ".MAIN_DB_PREFIX."societe as s,".MAIN_DB_PREFIX."facture as f";
...
if (! empty($conf->fournisseur->enabled) && $user->rights->fournisseur->facture->lire)
{
	...
	$sql = "SELECT ff.rowid, ff.fk_currency, ff.facnumber, ff.fk_statut, ff.libelle, ff.total_ht, ff.total_ttc, ff.paye,";
	$sql.= " s.nom, s.rowid as socid,";
	$sql.= " sum(pf.amount) as am,";
	//FEDE
	$sql.= "(SELECT ff.total_ht*rate FROM llx_currency_conversion WHERE source=ff.fk_currency AND target='".$conf->currency."' and date=(SELECT MAX(date) FROM llx_currency_conversion WHERE source=ff.fk_currency AND target='".$conf->currency."')) as Total, ";
	$sql.= "(SELECT ff.total_ttc*rate FROM llx_currency_conversion WHERE source=ff.fk_currency AND target='".$conf->currency."' and date=(SELECT MAX(date) FROM llx_currency_conversion WHERE source=ff.fk_currency AND target='".$conf->currency."')) as TotalTtc ";
	//FIN FEDE
	$sql.= " FROM ".MAIN_DB_PREFIX."societe as s, ".MAIN_DB_PREFIX."facture_fourn as ff";
	$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."paiementfourn_facturefourn as pf on ff.rowid=pf.fk_facturefourn";
	if (!$user->rights->societe->client->voir && !$socid) $sql.= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc";
	...
	if ($resql)
	{
		...
		print '<table class="noborder" width="100%">';
		print '<tr class="liste_titre"><td colspan="3">'.$langs->trans("BillsSuppliersUnpaid",$num).' <a href="'.DOL_URL_ROOT.'/fourn/facture/impayees.php">('.$num.')</a></td>';
		if (! empty($conf->global->MAIN_SHOW_HT_ON_SUMMARY)) print '<td align="right">'.$langs->trans("AmountHT").'</td>';
		print '<td align="right">'.$langs->trans("AmountTTC").'</td>';
		print '<td align="right">'.$langs->trans("Paid").'</td>';
		print '<td width="16">&nbsp;</td>';
		...
		if ($num)
		{
			...
			while ($i < $num)
			{
				...
				print '</tr>';
				$total += $obj->Total;
				$total_ttc +=  $obj->TotalTtc;
				$totalam +=  $obj->am;
				$i++;
				$var = !$var;
			}

			print '<tr class="liste_total"><td colspan="3">'.$langs->trans("Total").' en '.$langs->trans('Currency'.$conf->currency).' &nbsp; <font style="font-weight: normal">('.$langs->trans("RemainderToPay").': '.price($total_ttc-$totalam).')</font> </td>';
			if (! empty($conf->global->MAIN_SHOW_HT_ON_SUMMARY)) print '<td align="right">'.price($total).'</td>';
			print '<td align="right">'.price($total_ttc).'</td>';
			print '<td align="right">'.price($totalam).'</td>';
			print '<td>&nbsp;</td>';
